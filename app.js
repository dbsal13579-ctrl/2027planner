...
function getSubjColor(name){
    ...
    if(key==='기타' && bgTable['기타']) return bgTable['기타']; // 컬러표 테마: 6번째 색을 '기타' 과목에도 사용
...
function paletteTextOn(bgHex){
    ...
    return paletteColor(bgHex, .55, .20, Math.min(.32, Math.max(.20, bgL-.45)), .78);
}
function getSubjTextColor(name){
    ...
    const key = normalizeTimelineSubject(name);
    if(key==='기타' && bgTable['기타']) return paletteTextOn(bgTable['기타']);
    const baseHex = THEMES[currentTheme]?.solid || THEMES[currentTheme]?.color || THEMES.clearsky.color;
    const tint = TIMELINE_SUBJECT_TINTS[key] ?? TIMELINE_SUBJECT_TINTS['기타'];
...
// ★ 127 회독법 — 1일 전/2일 전/7일 전에 "완료"로 기록된 항목을 오늘 복습 추천으로 모아줌
//   (완료 기준은 일간 플래너와 동일: 체크박스 완료 또는 실제값 입력됨)
function getReview127Suggestions(dateStr){
    const offsets=[1,2,7];
    const results=[];
    offsets.forEach(off=>{
        const d=new Date(`${dateStr}T00:00:00`); d.setDate(d.getDate()-off);
        const ds=toKey(d);
        const day=scheduleData.find(x=>x.date===ds);
        if(!day || !day.tasks?.length) return;
        day.tasks.forEach((t,idx)=>{
            const key=`${ds}-${idx}`;
            const isDone=!!completedTasks[key];
            const hasActual = pageActuals[key]!==undefined && pageActuals[key]!=='';
            if(!isDone && !hasActual) return; // ★ 일간 플래너의 "완료" 판정 기준과 동일
            const sub=getTaskSubject(t);
            const txt=t.replace(/^\[.*?\]\s*/,'');
            results.push({daysAgo:off, date:ds, subject:sub||'기타', text:txt});
        });
    });
    return results;
}
...
const conditionCardHtml = `<div class="card p-4 mb-4">
    ...
    const review127 = getReview127Suggestions(selectedDate);
    const review127Html = review127.length ? `<div class="card p-4 mb-4">
        <h3 class="text-[14px] font-black text-slate-500 mb-2">🔁 오늘의 복습 추천 (127 회독법)</h3>
        <div class="space-y-1.5">
            ${review127.map(r=>`<div class="flex items-center gap-2 text-[13px]">
                <span class="font-black shrink-0" style="color:${getSubjTextColor(r.subject)||'var(--pc)'}">${r.subject}</span>
                <span class="font-semibold text-slate-600 truncate flex-1">${r.text}</span>
                <span class="text-slate-300 font-bold text-[11px] shrink-0">${r.daysAgo}일 전</span>
            </div>`).join('')}
        </div>
    </div>` : '';

  let html=`${conditionCardHtml}${review127Html}<div id="progress-summary-anchor"></div><div class="nav-box mb-5">
    <div class="nav-box-inner">
