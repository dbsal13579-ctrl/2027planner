...
function renderTimeline(){
    ...
    const hourLabelFont = isDayTimeline ? 12 : 11;
    const blockInset = 0;
    ...
    for(let h=minHour;h<maxHour;h++){
        ...
        rows.push(`<div style="display:flex;align-items:center;">
            <div style="width:${hourLabelW}px;text-align:right;padding-right:${isDayTimeline?5:4}px;font-size:${hourLabelFont}px;font-weight:800;color:#475569;flex-shrink:0;">${hourLabel}</div>
...
function renderRecordsMonth(container){
    ...
    for(let d=1;d<=days;d++){
        ...
        calHtml+=`<div class="heat-cell" style="background:${bg};${isToday?`outline:2px solid var(--pc);outline-offset:-2px;`:''}">
            <span style="font-size:12px;font-weight:${isToday?800:600};color:#0f172a">${d}</span>
            ${hText?`<span style="font-size:15px;font-weight:900;color:#0f172a">${hText}</span>`:''}
        </div>`;
    }
    calHtml+=`</div>`;
...
<div class="day-main-grid">
    ...
    day.tasks.forEach((t,idx)=>{
        ...
        const checkboxHtml = !pages ? `<input type="checkbox" ${isDone?'checked':''} onchange="toggleTask('${day.date}',${idx})" class="w-5 h-5 cursor-pointer shrink-0" style="accent-color:var(--pc)">` : '';
        ...
        const overCapBg = pct===100 ? `background:${pcAlpha(0.07)};border-radius:__RAD__;` // ★ 100% 정확히 달성
            : pct>100 ? `background:${pcAlpha(0.15)};border-radius:__RAD__;` // ★ 100% 초과
            : '';
    ...
    dayGroupKeys.forEach(groupKey=>{
        ...
        const groupOverCapBg = !allItemsAtLeast100 ? ''
            : anyItemOver100 ? `background:${pcAlpha(0.26)};border-radius:14px;`
            : `background:${pcAlpha(0.14)};border-radius:14px;`;
        html+=`<div class="day-subject-group">
            <div class="day-subject-header" style="${groupOverCapBg}">
            ...
            <div class="day-subject-tasks" style="${dayTasksCollapsed?'display:none':''}">
                ${g.rows.map((rowHtml,i)=>{
                    ...
                    const prevColored = i>0 && g.pcts[i-1]>=100;
    ...
    <div class="space-y-4">
        <div class="card timeline-focus-card">
            <div class="flex items-center" style="min-height:0;padding-top:8px;padding-left:6px;">
                <h3 class="panel-title" style="color:var(--pc);font-size:17px;margin:0;">오늘의 공부 흐름</h3>
            </div>
