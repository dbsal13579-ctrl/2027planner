...
function renderRecordsWeek(container){
    ...
    for(const[n,sec] of sorted){
        ...
        subRows+=`<div class="flex items-center gap-3 mb-2.5">
            <div class="text-[15px] font-black text-slate-600 w-20 shrink-0 truncate">${n}</div>
            <div class="flex-1 prog-bg" style="height:10px;border-radius:4px">
                <div class="prog-fill" style="width:${barPct}%;height:100%;border-radius:4px"></div>
            </div>
            <div class="text-[15px] font-black text-pc w-14 text-right">${h}</div>
        </div>`;
    }
...
const CAL_ALPHA_STOPS=[0.08,0.26,0.40,0.55,0.72];
...
function renderRecordsMonth(container){
    ...
    for(let d=1;d<=days;d++){
        ...
        calHtml+=`<div class="heat-cell" style="background:${bg};${isToday?`outline:2px solid var(--pc);outline-offset:-2px;`:''}">
            <span style="font-size:13px;font-weight:${isToday?800:600};color:#0f172a">${d}</span>
...
<div class="day-main-grid">
    ...
    day.tasks.forEach((t,idx)=>{
        ...
        const boxesHtml=!pages
            ? `<span class="day-task-page-meta day-task-boxes" style="${taskControlsCollapsed?'display:none':''}">
                <span class="day-task-meta day-task-placeholder">999p /</span>
            ...
            : `<span class="day-task-page-meta day-task-boxes" style="${taskControlsCollapsed?'display:none':''}">
                ...
                <input type="number" value="${actualVal}" placeholder="실제"
                    ...
                    class="day-task-actual"><span class="day-task-meta">${pages.unit}</span>
            </span>`;
        const percentHtml=`<span class="day-task-percent">${pages?pct:(isDone?100:0)}%</span>`;
        const overCapBg = pct===100 ? 'background:transparent;' // ★ 100% 정확히 달성 = 미래(아직 안 온) 요일과 같은 배경(투명)
    ...
    dayGroupKeys.forEach(groupKey=>{
        ...
        // ★ 이 과목의 모든 하위 항목이 100% 이상 달성했을 때만 그룹 전체에도 같은 배경 규칙 적용
        const allItemsAtLeast100 = g.pcts.length>0 && g.pcts.every(p=>p>=100);
        const groupOverCapBg = !allItemsAtLeast100 ? ''
            : avgPct===100 ? 'background:transparent;'
            : 'background:linear-gradient(180deg, rgba(var(--pc-rgb),.11) 0%, rgba(var(--pc-rgb),.05) 100%);';
    ...
    <div class="space-y-4">
        <div class="card timeline-focus-card">
            ...
            <div class="flex items-end justify-between mt-2 flex-wrap gap-1">
                <h3 class="panel-title" style="color:var(--pc)">오늘의 공부 흐름</h3>
                <div class="text-[13px] font-bold text-slate-400">${renderDayTimeBucketsLabel(selectedDate)}</div>
            </div>
