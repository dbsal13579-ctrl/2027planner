...
function renderProgressLectureView(container){
    ...
    let cellsHtml='';
    if(item.startDate && item.deadline){
        let d=new Date(`${item.startDate}T00:00:00`);
        const end=new Date(`${item.deadline}T00:00:00`);
        let cursor=0;
        while(d<=end){
            const ds=toKey(d);
            const reachedThatDay = rec.dailyReached[ds];
            const hasData = reachedThatDay!==undefined;
            cursor = hasData ? reachedThatDay : cursor;
            cellsHtml+=`<div class="rounded-lg text-center py-2 text-[11px] font-bold" style="background:${hasData?pcAlpha(0.14):'#f8fafc'};color:${hasData?'var(--pc-solid)':'#cbd5e1'}">
                <div style="font-size:9px;opacity:.7;">${ds.slice(5)}</div>
                <div>${hasData?cursor:'-'}</div>
            </div>`;
            d.setDate(d.getDate()+1);
        }
    }
    ...
    container.innerHTML=`<div class="card p-5">
        ...
        <div class="text-xs text-slate-400 mb-3 font-bold">
            ${rec.firstDate?`시작 ${rec.firstDate} · ${rec.completeDate?`완료 ${rec.completeDate}`:`진행중 (${rec.current}/${item.total}${item.unit})`}`:'아직 기록 없음'}
        </div>
        <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px;">${cellsHtml || '<p class="text-slate-300 text-sm col-span-7 text-center py-6">시작일/마감일이 설정되지 않았어요</p>'}</div>
    </div>`;
}
...
function renderSubjectDonutChart(sorted, totalSec){
    ...
    return `<div class="flex items-center gap-6 flex-wrap">
        <div style="width:140px;height:140px;border-radius:50%;background:conic-gradient(${gradient});flex-shrink:0;position:relative;">
            <div style="position:absolute;inset:12px;background:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-direction:column;">
...
<div class="day-main-grid">
    ...
    <div class="space-y-4">
        <div class="card timeline-focus-card">
            <div class="flex items-center" style="min-height:0;padding-top:8px;padding-left:10px;">
