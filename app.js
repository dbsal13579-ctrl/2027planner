...
function autoScheduleWeek(){
    ...
    progressItems.forEach(item=>{
        ...
        let cursor=prog.current; // ★ 지금 이 순간의 실제 진행 지점(=오늘까지 실제 기록한 최종 도달치)부터 이어서 계획을 짠다
        ...
        horizonDates.forEach(dateStr=>{
            ...
            if(planIdxs.length){
                ...
                if(userIdxs.length){
                    // ★ 사용자가 직접 추가/수정한 범위(예: 202~250p)는 그대로 존중하고,
                    //   그 뒤 날짜의 자동 계획은 이 범위의 끝에서 이어지게 cursor만 앞으로 옮긴다.
                    userIdxs.forEach(i=>{
                        const p=parseTaskPages(existingDay.tasks[i]);
                ...
                // 손대지 않은 자동 제안만 있는 경우: cursor랑 이어지면 유지, 아니면 낡은 계획이므로 지우고 새로 짠다
                const p=parseTaskPages(existingDay.tasks[autoIdxs[0]]);
                if(autoIdxs.length===1 && p && p.start-1===cursor){
                ...
                changed=true;
                // 아래로 통과시켜서 이 날짜를 지금 cursor 기준으로 새로 배치한다
            }
            ...
            if(!day.tasks.some(isPlanTask)){
                const text=`${prefix} ${cursor+1}~${end}${item.unit}`;
                day.tasks.push(text);
    ...
    sorted.forEach(day=>{
        ...
        (day.tasks||[]).forEach((t,idx)=>{
            ...
            if(reached>0){
                ...
                if(current>=item.total && !completeDate) completeDate=day.date;
            }
        });
    });
    return {firstDate, completeDate, current, dailyReached};
}
...
function renderProgressLectureView(container){
    ...
    const sortedDates=Object.keys(rec.dailyReached).sort();
    let prev=0;
    sortedDates.forEach(ds=>{
        const r=rec.dailyReached[ds];
        for(let c=prev+1; c<=Math.min(r,item.total); c++) countDate[c]=ds;
        if(r>prev) prev=r;
    });
    ...
    for(let n=1; n<=item.total; n++){
        ...
        cellsHtml+=`<div class="rounded-md text-center py-1.5" style="background:${ds?pcAlpha(0.14):'#f8fafc'};color:${ds?'var(--pc-solid)':'#cbd5e1'};font-size:10px;font-weight:800;line-height:1.3;">
            <div>${ds?ds.slice(5).replace('-','.'):n}</div>
        </div>`;
    }
    ...
    container.innerHTML=`<div class="card p-5">
        ...
        <div class="text-xs text-slate-400 mb-3 font-bold">
            ${rec.firstDate?`시작 ${rec.firstDate} · ${rec.completeDate?`완료 ${rec.completeDate} (${daysLabel}일 소요)`:`진행중 (${rec.current}/${item.total}${item.unit})`}`:'아직 기록 없음'}
        </div>
        ...
        const itemsHtml=list.map(c=>{
            ...
            return `<div class="py-2 border-b border-slate-50 last:border-0">
                ...
                <div class="flex items-center gap-2">
                    <div class="text-[13px] font-semibold text-slate-500 shrink-0" style="min-width:90px;">${current}/${item.total}${item.unit} · 남음 ${remaining}${item.unit}</div>
