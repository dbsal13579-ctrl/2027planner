...
function renderTimeline(){
    ...
    const rowHeight = isDayTimeline ? 26 : 20;
    const rowMargin = 1;
    const rowRadius = 6;
    const hourLabelW = isDayTimeline ? 24 : 22;
...
<div class="day-main-grid">
        ...
        <div class="card day-task-card">
            <div class="flex gap-2 mb-4 items-center">
                ...
                <input id="new-task-input" type="text" placeholder="[과목] 할 일을 입력"
                    class="flex-1 min-w-0 bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-2 outline-none focus:border-pc font-semibold transition-colors"
                    style="font-size:17px;"
                    onkeydown="if(event.key==='Enter')addTask()">
                <button onclick="addTask()" class="shrink-0 bg-pc text-white px-5 py-2 rounded-2xl font-black hover:opacity-80 transition-all" style="font-size:15px;">추가</button>
            </div>
            <div>`;
    ...
    day.tasks.forEach((t,idx)=>{
        ...
        const overCapBg = pct===100 ? `background:${pcAlpha(0.14)};border-radius:14px;` // ★ 100% 정확히 달성 (기준값 20%의 70%)
            : pct>100 ? `background:${pcAlpha(0.26)};border-radius:14px;` // ★ 100% 초과 (기준값 20%의 130%)
            : '';
    ...
    <div class="space-y-4">
        <div class="card timeline-focus-card">
            <div class="flex items-center" style="min-height:0;">
                <h3 class="panel-title" style="color:var(--pc);font-size:17px;margin:0;">오늘의 공부 흐름</h3>
            </div>
            <div class="text-[13px] font-bold text-slate-400 text-right" style="margin-top:0;">${renderDayTimeBucketsLabel(selectedDate)}</div>
