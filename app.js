...
function autoScheduleWeek(){
    ...
    progressItems.forEach(item=>{
        ...
        horizonDates.forEach(dateStr=>{
            ...
            if(excludedAutoDates[item.id]?.includes(dateStr)) return; // ★ 사용자가 이 날짜에서 직접 삭제했던 경우 — 다시 채우지 않음

            const existingDay=scheduleData.find(s=>s.date===dateStr);
            const existingIdx=existingDay?.tasks.findIndex(t=>t.startsWith(prefix+' ')) ?? -1;
            const existingTask=existingIdx>=0 ? existingDay.tasks[existingIdx] : null;

            if(existingTask){
                const actualKey=`${dateStr}-${existingIdx}`;
                ...
                // 아직 실제 기록은 없는데(=오늘 이후 미래 계획), cursor랑 안 맞으면 이 태스크는 낡은 계획 → 지우고 새로 짠다
                const p=parseTaskPages(existingTask);
                if(p && p.start-1===cursor){
                    cursor=Math.max(cursor,p.end); // 이미 맞게 이어져 있으면 그대로 유지
                    return;
                }
                existingDay.tasks=existingDay.tasks.filter(t=>t!==existingTask);
                reindexDayRecordsAfterRemoval(dateStr, existingIdx);
...
function addTimelineNote(){
    ...
    const label = prompt('무엇을 했나요? (예: 밥, 운동)');
    if(!label) return;
    const startStr = prompt('시작 시각 (예: 12:40)');
    if(!startStr || !/^\d{1,2}:\d{2}$/.test(startStr.trim())){ showToast('⏱ 시간 형식이 올바르지 않아요 (예: 12:40)'); return; }
    const endStr = prompt('종료 시각 (예: 12:50)');
    if(!endStr || !/^\d{1,2}:\d{2}$/.test(endStr.trim())){ showToast('⏱ 시간 형식이 올바르지 않아요 (예: 12:50)'); return; }
    const toSec=(s)=>{ const [h,m]=s.trim().split(':').map(Number); return h*3600+m*60; };
    let startSec=toSec(startStr), endSec=toSec(endStr);
    if(startSec<DAY_START_TIMELINE_SEC) startSec+=86400;
    if(endSec<DAY_START_TIMELINE_SEC) endSec+=86400;
    if(endSec<=startSec){ showToast('⏱ 종료 시각이 시작 시각보다 뒤여야 해요'); return; }
    if(Math.floor(startSec/3600)!==Math.floor((endSec-1)/3600)){ showToast('⏱ 지금은 같은 시(예: 12시대) 안에서만 메모를 추가할 수 있어요'); return; }
    const notes=getTimelineNotes(dateStr);
    notes.push({label:label.trim(), startSec, endSec});
    saveTimelineNotes(dateStr, notes);
    renderTimeline();
}
function removeTimelineNote(idx){
    const dateStr = typeof timerDate==='string' ? timerDate : TODAY;
    const notes=getTimelineNotes(dateStr);
    notes.splice(idx,1);
    saveTimelineNotes(dateStr, notes);
...
function renderTimelineNotesOverlay(h, hSec, HOUR){
    ...
    return notes.map((note,idx)=>{
        ...
        return `<div onclick="if(confirm('메모 [${note.label}] 를 지울까요?'))removeTimelineNote(${idx})"
            ...
            ${showLabel?`<div style="font-size:9px;font-weight:900;color:#64748b;background:#fff;padding:0 3px;margin-top:1px;border-radius:4px;white-space:nowrap;">${note.label}</div>`:''}
        </div>`;
    }).join('');
}
...
function copyUnfinishedTasksFromOtherDay(){
    const defaultSrc = toKey(new Date(new Date(`${selectedDate}T00:00:00`).getTime()-86400000));
    const srcStr = prompt(`어느 날짜에서 안 한 일을 가져올까요? (예: ${defaultSrc})`, defaultSrc);
    if(!srcStr || !/^\d{4}-\d{2}-\d{2}$/.test(srcStr.trim())){
        if(srcStr!==null) showToast('📅 날짜 형식이 올바르지 않아요 (예: 2026-07-21)');
        return;
    }
    const srcDate = srcStr.trim();
    const srcDay = scheduleData.find(d=>d.date===srcDate);
    if(!srcDay || !srcDay.tasks?.length){ showToast('📭 그 날짜엔 할 일이 없어요'); return; }

    const unfinished=[];
    srcDay.tasks.forEach((t,idx)=>{
        const completion=getTaskCompletion(srcDate, idx, t);
        if(completion<1) unfinished.push(t.replace(/^\[.*?\]\s*/,'').trim()===t.trim() ? t : t); // 태그 포함 원문 유지
    });
    // 완료율이 100% 미만인 것만 다시 필터 (위에서 이미 걸렀지만 페이지형 실제값도 제외하기 위해 원문 그대로 사용)
    if(!unfinished.length){ showToast('🎉 그 날은 다 완료했어요!'); return; }

    let day=scheduleData.find(d=>d.date===selectedDate);
    if(!day){ day={date:selectedDate, tasks:[]}; scheduleData.push(day); }

    let addedCount=0;
    unfinished.forEach(t=>{
        // 페이지/회차형 태스크는 실제값·체크 상태 없이 "목표"만 그대로 복사(새 진행 상황으로 시작)
        if(!day.tasks.includes(t)){
            day.tasks.push(t);
            addedCount++;
        }
    });
    saveAll();
    renderPlanner();
    showToast(`✅ ${srcDate}의 안 한 일 ${addedCount}개를 가져왔어요`);
}

function deleteTask(idx){
    const d=scheduleData.find(d=>d.date===selectedDate);if(!d)return;
    const removedText=d.tasks[idx];
    // ★ 삭제한 태스크가 등록된 진도 항목과 매칭되면, 이 날짜는 다시 자동으로 채우지 않도록 기억해둔다
    if(removedText){
        const sub=getTaskSubject(removedText);
        const txt=removedText.replace(/^\[.*?\]\s*/,'');
        const item=findMatchingProgressItem(sub, txt);
        if(item){
            if(!excludedAutoDates[item.id]) excludedAutoDates[item.id]=[];
            if(!excludedAutoDates[item.id].includes(selectedDate)) excludedAutoDates[item.id].push(selectedDate);
        }
    }
    ...
    for(let d=1;d<=days;d++){
        ...
        html+=`<div class="cal-cell${isToday?' is-today':''}${monthShowAllTasks?' cal-cell-expanded':''}" style="background:${cellBg};" onclick="goToDay('${key}')">
            <div class="cal-cell-summary">
                ...
                ${isOff?`<span class="block ${isBareOff?'mt-3 text-[26px]':'mt-1.5 text-[10px]'} leading-none font-black tracking-wide" style="color:#94a3b8;">Day off</span>`:''}
                ${(!isOff || timeSec>0) ? `
...
<div class="day-main-grid">
    <div class="space-y-4">
        <div class="card day-task-card">
            ...
            <button onclick="copyUnfinishedTasksFromOtherDay()" class="w-full mb-4 text-[12px] font-bold py-2 rounded-xl text-slate-400 hover:text-pc transition-colors" style="background:#f8fafc;border:1px solid #eef2f6;">↩ 다른 날 안 한 일 가져오기</button>
            <div>`;
