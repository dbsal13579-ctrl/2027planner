...
function cleanupStaleAutoPlacedTasks(){
    ...
    try{
        ...
        scheduleData.forEach(day=>{
            ...
            const before=day.tasks.length;
            day.tasks=day.tasks.filter((t,idx)=>{
                const sub=getTaskSubject(t);
                const txt=t.replace(/^\[.*?\]\s*/,'');
                const item=findMatchingProgressItem(sub, txt);
...
function catchUpTimerOnResume(){
    ...
    timerSubjects=timerSubjects.map(s=>{
        ...
        let elapsed=nowSec-s._startSec;
        if(elapsed<0) elapsed+=86400; // 자정을 넘긴 경우 보정
        if(elapsed<=0) return s;
        changed=true;
        // ★ 흐름 차트(오늘의 공부 흐름)에도 이 구간이 남도록 로그를 기록해 둔다
        timerLogs.push({n:s.n, startSec:s._startSec, endSec:nowSec});
        logsChanged=true;
        return {...s, spent:s.spent+elapsed, rem:Math.max(0,s.rem-elapsed), _startSec:nowSec};
    });
    if(logsChanged) saveTimerLogs();
    if(changed){
        renderTimer();
        updateLiveDayTimerDisplay();
        renderTimeline();
...
function renderRecordsMonth(container){
    ...
    // max hours for color scale — ★ 상대값이 아니라 절대 시간 기준 5단계(1/4/7/10/12시간)로 고정
    const HOUR_STOPS=[1,4,7,10,12]; // 시간 단위
    const ALPHA_STOPS=[0.16,0.32,0.48,0.64,0.85];
    function alphaForSecs(secs){
        if(secs<=0) return 0;
        const hrs=secs/3600;
        for(let i=0;i<HOUR_STOPS.length;i++){
            if(hrs<HOUR_STOPS[i]) return i===0 ? ALPHA_STOPS[0]*(hrs/HOUR_STOPS[0]) : ALPHA_STOPS[i];
        }
        return ALPHA_STOPS[ALPHA_STOPS.length-1];
    }
    ...
    calHtml+=`<div class="flex items-center gap-2 mt-3 justify-end flex-wrap">
        ${ALPHA_STOPS.map((a,i)=>`<div class="flex items-center gap-1">
            <div style="width:14px;height:14px;border-radius:3px;background:${pcAlpha(a)}"></div>
            <span class="text-[10px] text-slate-400">${HOUR_STOPS[i]}h</span>
        </div>`).join('')}
    </div>`;
