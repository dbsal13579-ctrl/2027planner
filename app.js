...
function startTimerTick(){
    ...
    timerInterval=setInterval(()=>{
        if(checkTimerDateRollover()) return; // 자정 넘겨 05시가 되면 날짜 전환 후 이번 tick은 스킵
        let changed=false;
...
function catchUpTimerOnResume(){
    if(checkTimerDateRollover()) return; // 자정을 넘겨 05시가 지났으면 날짜부터 전환하고 끝
...
function getNowDaySec(){
    // ★ "공부일" 기준(오늘 05:00~다음날 04:59) 초 단위 좌표.
    //   timerDate가 가리키는 공부일이 아직 "오늘"(실제 캘린더 날짜)이면 0~86399초로,
    //   실제 캘린더 날짜가 timerDate보다 하루 지났고(=자정을 넘겨서 새벽까지 이어진 상태)
    //   아직 05시 이전이면 24~29시(86400~104399초)로 환산해서 자정을 넘어도 시간이
    //   계속 증가하도록 한다. (자정에 초가 0으로 리셋되어 시작-종료가 역전되는 문제 방지)
    const now=new Date();
    const nowSec=now.getHours()*3600+now.getMinutes()*60+now.getSeconds();
    const realDateStr=localDateStr(now);
    if(typeof timerDate==='string' && realDateStr>timerDate && nowSec<5*3600){
        return nowSec+86400;
    }
    return nowSec;
}
// ★ 자정을 넘겨 공부를 계속하다 새벽 05시를 지나면, 화면에 보이는 timerDate가
//   "오늘"(실제 캘린더 날짜)로 자동 전환되도록 한다. 이게 없으면 0~4시대에 기록한
//   시간이 계속 "어제" 날짜 밑에 쌓이다가 자정 롤오버 시 시작-종료 시각이 역전돼
//   기록이 깨지거나 화면에 반영되지 않는다.
function checkTimerDateRollover(){
    if(typeof timerDate!=='string') return false;
    const now=new Date();
    const realDateStr=localDateStr(now);
    const nowSec=now.getHours()*3600+now.getMinutes()*60+now.getSeconds();
    if(realDateStr>timerDate && nowSec>=5*3600){
        // 넘어가기 직전(옛 timerDate 기준 24~29시대 값)까지를 "어제" 로그로 끊어 저장하고,
        // 켜져 있던 과목 이름들은 기억해뒀다가 새 날짜(오늘)에서 그대로 이어 켠다.
        const cutSec=getNowDaySec(); // 아직 timerDate가 안 바뀐 시점이므로 24~29시대 값
        const runningNames=timerSubjects.filter(s=>s.on && s._startSec!==undefined).map(s=>s.n);
        runningNames.forEach(n=>{
            const s=timerSubjects.find(x=>x.n===n);
            timerLogs.push({n:s.n,startSec:s._startSec,endSec:cutSec});
        });
        if(runningNames.length) saveTimerLogs();

        loadTimerDay(realDateStr); // timerDate=오늘로 전환, 오늘 로그/과목 새로 로드

        if(runningNames.length){
            const resumeStartSec=0; // 오늘(05시 이전 새벽 구간) 기준 자정=0초 지점부터 이어서 시작
            timerSubjects=timerSubjects.map(s=>
                runningNames.includes(s.n)
                    ? {...s,on:true,_startSec:resumeStartSec,_lastCreditedSec:undefined}
                    : s
            );
            if(!timerInterval) startTimerTick();
            renderTimer();
...
function renderTimeline(){
    ...
    //   (자기 전까지 공부한 걸 "오늘"로 치는 개념이라 05시 이전 기록은 어제 몫으로 빼고,
    //    다음날 0~5시 기록을 오늘 뒤에 이어붙인다)
    const DAY_START_SEC = 5*3600;
    let studyNowSec = nowSec; // 실시간 표시용 "지금" 좌표(오늘 05시 이전이면 +86400 보정)
    if(isDayTimeline && typeof timerDate==='string'){
        allLogs = allLogs.filter(l => l.endSec > DAY_START_SEC); // 05시 이전에 끝난 기록은 어제 몫
        allLogs = allLogs.map(l => ({...l, startSec: Math.max(l.startSec, DAY_START_SEC)}));
        try{
            const nextDateStr = toKey(new Date(new Date(`${timerDate}T00:00:00`).getTime()+86400000));
            let nextLogs = JSON.parse(localStorage.getItem(`yst4_log_${nextDateStr}`))||[];
            nextLogs = nextLogs.filter(l=>l.startSec<DAY_START_SEC)
                .map(l=>({...l, startSec:l.startSec+86400, endSec:Math.min(l.endSec,DAY_START_SEC)+86400}));
            allLogs = allLogs.concat(nextLogs);
        }catch{}
        if(nowSec<DAY_START_SEC) studyNowSec = nowSec+86400;
    }
...
function computeDayTimeBuckets(dateStr){
    ...
    let todayLogs=[], nextLogs=[];
    try{ todayLogs=JSON.parse(localStorage.getItem(`yst4_log_${dateStr}`))||[]; }catch{}
    try{
        const nextDateStr=toKey(new Date(new Date(`${dateStr}T00:00:00`).getTime()+86400000));
        nextLogs=JSON.parse(localStorage.getItem(`yst4_log_${nextDateStr}`))||[];
    }catch{}
    ...
    // ★ "공부일" 개념(오늘 05:00~다음날 05:00)에 맞춰 오전(5:00~12:30)/오후(12:31~20:00)/밤(20:01~다음날 5:00)으로
    //   하루를 빈틈없이 나눠서, 세 구간의 합이 항상 그날의 전체 공부시간과 정확히 일치하도록 한다.
    const morning = sumRange(todayLogs, 5*3600, 12*3600+30*60);
    const afternoon = sumRange(todayLogs, 12*3600+31*60, 20*3600);
    const night = sumRange(todayLogs, 20*3600+60, 86400) + sumRange(nextLogs, 0, 5*3600);
...
function renderRecordsWeek(container){
    ...
    for(const[n,sec] of sorted){
        ...
        const subjColor=getSubjColor(n)||'var(--pc)';
        const subjTextColor=getSubjTextColor(n)||'var(--pc)';
        subRows+=`<div class="flex items-center gap-3 mb-2.5">
            <div class="text-[15px] font-black w-20 shrink-0 truncate" style="color:${subjTextColor}">${n}</div>
            <div class="flex-1 prog-bg" style="height:10px;border-radius:4px">
                <div style="width:${barPct}%;height:100%;border-radius:4px;background:${subjColor}"></div>
            </div>
            <div class="text-[15px] font-black w-14 text-right" style="color:${subjTextColor}">${h}</div>
        </div>`;
    }
