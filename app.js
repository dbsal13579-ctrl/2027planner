...
function collapseArrowHtml(onclickFn, isOpen, extraStyle){
    return `<button onclick="${onclickFn}" style="background:transparent;border:none;color:#cbd5e1;font-size:12px;font-weight:900;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;transform:rotate(${isOpen?90:0}deg);transition:transform .15s;${extraStyle||''}">▶</button>`;
}
...
function alphaForSecs(secs){
    if(secs<=0) return 0;
    const hrs=secs/3600;
    for(let i=0;i<CAL_HOUR_STOPS.length;i++){
        if(hrs<CAL_HOUR_STOPS[i]) return i===0 ? CAL_ALPHA_STOPS[0]*(hrs/CAL_HOUR_STOPS[0]) : CAL_ALPHA_STOPS[i];
    }
    return CAL_ALPHA_STOPS[CAL_ALPHA_STOPS.length-1];
}
...
<div class="flex items-center justify-between mb-2 flex-wrap gap-2">
    ...
    <div class="flex items-center gap-3 flex-wrap ml-auto">
        <div class="font-black" style="color:var(--pc);font-size:15px;">${getThemeAccent()} ${year}.${String(month+1).padStart(2,'0')}</div>
        ${calColorLegendHtml()}
    </div>
</div>`;
...
<div class="day-main-grid">
    ...
    day.tasks.forEach((t,idx)=>{
        ...
        const pgUI=pages
            ...
            : '';
        const overCapBg = pct>100 ? 'background:transparent;' : ''; // ★ 100% 초과 시 주간뷰의 '아직 안 온 요일'과 같은(강조 없는 투명) 배경
