...
function renderTimeline(){
    ...
    const rowRadius = isDayTimeline ? 6 : 3;
...
const CAL_ALPHA_STOPS=[0.08,0.16,0.24,0.32,0.42];
...
<div class="day-main-grid">
    <div class="space-y-4">
        <div class="card day-task-card">
            <div class="flex gap-3 mb-4">
                ${collapseArrowHtml('toggleDayTasksCollapse()', !dayTasksCollapsed, 'width:16px;')}
                <button onclick="toggleTaskControlsCollapse()" title="목표/실제 입력칸 접기·펼치기" class="shrink-0" style="background:transparent;border:none;color:#cbd5e1;font-size:12px;font-weight:900;cursor:pointer;width:16px;">${taskControlsCollapsed?'▤':'▦'}</button>
    ...
    day.tasks.forEach((t,idx)=>{
        ...
        const checkPctUI=!pages
            ...
            : '';
        const pgUI=pages
            ? `<span class="day-task-page-meta"><span class="day-task-meta">
            ...
            : '';
        const overCapBg = pct>100 ? 'background:#f1f5f9;' : ''; // ★ 100% 초과 시 눈에 띄는 연한 회색(주간뷰 미표시 상태와 같은 계열) 배경
        const rowHtml=`<div class="day-task-row day-task-row-grouped ${isDone?'is-done':''}" style="${overCapBg}">
            ${!pages?`<input type="checkbox" ${isDone?'checked':''} onchange="toggleTask('${day.date}',${idx})" class="w-5 h-5 cursor-pointer shrink-0" style="accent-color:var(--pc)">`
                    :`<div class="w-5 h-5 shrink-0" style="visibility:hidden"></div>`}
            <div class="min-w-0">
                <span class="day-task-title ${isDone?'line-through text-slate-400':'text-slate-700'}">${txt||t}</span>
                ${suggestionHtml}
            </div>
            <div class="day-task-controls" style="${taskControlsCollapsed?'display:none':''}">
                ${checkPctUI}
                ${pgUI}
            </div>
            <button onclick="deleteTask(${idx})" title="삭제" class="hover:text-red-400 px-2 py-1 rounded transition-colors shrink-0" style="color:#cbd5e1;font-size:12px;font-weight:900;">✕</button>
        </div>`;
