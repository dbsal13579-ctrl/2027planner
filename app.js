...
function renderDayTimeBucketsLabel(dateStr){
    ...
    return `오전 ${fmtHFromSec(b.morning)} / 오후 ${fmtHFromSec(b.afternoon)} / 밤 ${fmtHFromSec(b.night)} = ${fmtHFromSec(b.total)}`;
}
...
<div class="flex items-center justify-between mb-2 flex-wrap gap-2">
    <div class="text-left font-black" style="color:var(--pc);font-size:18px;">${getThemeAccent()} ${year}.${String(month+1).padStart(2,'0')}</div>
    ${calColorLegendHtml()}
</div>`;
        ...
        <div class="card day-task-card">
            <div class="flex gap-3 mb-4">
                <button onclick="toggleDayTasksCollapse()" title="${dayTasksCollapsed?'항목 펼치기':'항목 접기'}"
                    class="shrink-0 w-11 rounded-2xl border-2 border-slate-100 bg-slate-50 flex items-center justify-center text-slate-400 hover:text-pc hover:border-pc transition-colors text-lg"
                    style="transform:rotate(${dayTasksCollapsed?'-90deg':'0deg'});transition:transform .2s">▽</button>
