...
let dayTasksCollapsed = false; // 일간 플래너 — 과목별 세부 항목 전체 접기/펼치기 상태
function toggleDayTasksCollapse(){
    dayTasksCollapsed = !dayTasksCollapsed;
    renderPlanner();
}
let conditionCardCollapsed = false; // 일간 플래너 — 오늘 컨디션 카드 접기/펼치기 상태
...
function computeDayTimeBuckets(dateStr){
    ...
    const morning = sumRange(todayLogs, 7*3600, 12*3600+30*60);
...
const CAL_ALPHA_STOPS=[0.16,0.32,0.48,0.64,0.85];
...
function renderRecordsMonth(container){
    ...
    for(let d=1;d<=days;d++){
        ...
        calHtml+=`<div class="heat-cell" style="background:${bg};${isToday?`outline:2px solid var(--pc);outline-offset:-2px;`:''}">
            ...
            ${hText?`<span style="font-size:13px;font-weight:800;color:#0f172a">${hText}</span>`:''}
        </div>`;
    }
    calHtml+=`</div>`;
...
<div class="flex items-center justify-end mb-2 flex-wrap gap-2">
    ...
    positionProgressSummary('progress-summary-default-anchor');
};
...
renderDay=function(container){
    ...
    const conditionCardHtml = `<div class="card p-4 mb-4">
        <h3 class="text-[14px] font-black text-slate-500 mb-2 flex items-center gap-1.5">
            <button onclick="toggleConditionCardCollapse()" title="접기/펼치기"
                style="background:transparent;border:none;color:#94a3b8;font-size:11px;cursor:pointer;transition:transform .2s;transform:rotate(${conditionCardCollapsed?'-90deg':'0deg'});">▽</button>
    ...
    <div class="space-y-4">
        <div class="card day-task-card">
            <div class="flex gap-3 mb-4">
                <button onclick="toggleDayTasksCollapse()" title="${dayTasksCollapsed?'항목 펼치기':'항목 접기'}"
                    class="shrink-0 flex items-center justify-center text-slate-400 hover:text-pc transition-colors"
                    style="background:transparent;border:none;font-size:11px;width:16px;padding:0;transform:rotate(${dayTasksCollapsed?'-90deg':'0deg'});transition:transform .2s">▽</button>
    ...
    <div class="space-y-4">
        <div class="card timeline-focus-card">
            ...
            <div class="mt-2">
                ...
                <div class="text-[13px] font-bold text-slate-400 mt-1">${renderDayTimeBucketsLabel(selectedDate)}</div>
            </div>
