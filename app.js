...
const THEMES = {
    ...
    barbiedoll:  { name:'바비돌',      color:'#F94FA0', rgb:'249,79,160',  solid:'#F0157E', solidRgb:'240,21,126' },
    peachfizz:  { name:'피치피즈',      color:'#F4886A', rgb:'244,136,106',  solid:'#EE6845', solidRgb:'238,104,69' },
    summertime:  { name:'서머타임',      color:'#3FC9D8', rgb:'63,201,216',  solid:'#1AB6C7', solidRgb:'26,182,199' },
    macaronforest:  { name:'카카롱숲',      color:'#5C8F6E', rgb:'92,143,110',  solid:'#3F7A54', solidRgb:'63,122,84' },
    pinkrose:  { name:'핑크로즈',      color:'#C24D74', rgb:'194,77,116',  solid:'#A8335B', solidRgb:'168,51,91' },
    cafelatte:  { name:'카페라떼',      color:'#8B5E3C', rgb:'139,94,60',  solid:'#734A2C', solidRgb:'115,74,44' },
    vintagesky:  { name:'빈티지스카이',      color:'#6E7B8B', rgb:'110,123,139',  solid:'#57636F', solidRgb:'87,99,111' },
    taiga:  { name:'타이가',      color:'#7C93A8', rgb:'124,147,168',  solid:'#617888', solidRgb:'97,120,136' },
    cupcake:  { name:'컵케이크',      color:'#3FBBA3', rgb:'63,187,163',  solid:'#25A88D', solidRgb:'37,168,141' },
    summerocean:  { name:'서머오션',      color:'#1F8A8C', rgb:'31,138,140',  solid:'#116F70', solidRgb:'17,111,112' },
    oceandeep:  { name:'오션딥',      color:'#28417A', rgb:'40,65,122',  solid:'#1B2E5C', solidRgb:'27,46,92' },
    cookiencream:  { name:'쿠키앤크림',      color:'#9C8064', rgb:'156,128,100',  solid:'#846A4E', solidRgb:'132,106,78' },
    hellorustone:  { name:'헬로러스톤',      color:'#6B7C93', rgb:'107,124,147',  solid:'#54637A', solidRgb:'84,99,122' },
    amethyst:  { name:'아메시스트',      color:'#8E6FAE', rgb:'142,111,174',  solid:'#75568F', solidRgb:'117,86,143' },
    heliotrope:  { name:'헬리오트로프',      color:'#4E7FC1', rgb:'78,127,193',  solid:'#3766A8', solidRgb:'55,102,168' },
    christmas:  { name:'크리스마스',      color:'#C23B3B', rgb:'194,59,59',  solid:'#A82727', solidRgb:'168,39,39' },
    dahlia:  { name:'달리아',      color:'#7C8A3D', rgb:'124,138,61',  solid:'#66732E', solidRgb:'102,115,46' },
    /* ── 컬러표(원본 이미지 색상 기준)로 추가한 테마: solid=이미지의 테마색(T), color=solid를 흰색과 18% 섞은 값 ── */
    hellolisbon:  { name:'헬로리스본', color:'#BBCAE5', rgb:'187,202,229', solid:'#ACBEDF', solidRgb:'172,190,223' },
    summertime2:  { name:'서머타임 2', color:'#7CD0DA', rgb:'124,208,218', solid:'#5FC6D2', solidRgb:'95,198,210' },
    ivywoods:  { name:'아이비우즈', color:'#A0A89D', rgb:'160,168,157', solid:'#8B9588', solidRgb:'139,149,136' },
    veryperi2:  { name:'베리페리 II', color:'#A2A3C5', rgb:'162,163,197', solid:'#8E8FB8', solidRgb:'142,143,184' },
    meganisi:  { name:'메가니시', color:'#76898A', rgb:'118,137,138', solid:'#586F70', solidRgb:'88,111,112' },
    denimlove:  { name:'데님러브', color:'#8EAABD', rgb:'142,170,189', solid:'#7597AE', solidRgb:'117,151,174' },
    sereneearth:  { name:'세린어스', color:'#87897E', rgb:'135,137,126', solid:'#6D6F62', solidRgb:'109,111,98' },
    cozycity:  { name:'코지시티', color:'#D8C8BF', rgb:'216,200,191', solid:'#CFBCB1', solidRgb:'207,188,177' },
    dreamysurf2:  { name:'드리미서프 2', color:'#DCC4CD', rgb:'220,196,205', solid:'#D4B7C2', solidRgb:'212,183,194' },
    grapefruit:  { name:'그레이프프루트', color:'#F49A42', rgb:'244,154,66', solid:'#F28419', solidRgb:'242,132,25' },
    /* ── 컬러표 2차 추가(원본 이미지 색상 기준). 타이가 2는 원본 대표색이 너무 옅어 같은 색조로 살짝 진하게 조정 ── */
    mapleleaves:  { name:'메이플리브스', color:'#C1635F', rgb:'193,99,95', solid:'#B3413C', solidRgb:'179,65,60' },
    rosetone:  { name:'로즈', color:'#DA6C7E', rgb:'218,108,126', solid:'#D24C62', solidRgb:'210,76,98' },
    taiga2:  { name:'타이가 2', color:'#C2D8DF', rgb:'194,216,223', solid:'#B4D0D8', solidRgb:'180,208,216' },
    pebble2:  { name:'페블 2', color:'#548995', rgb:'84,137,149', solid:'#2F6F7E', solidRgb:'47,111,126' },
    ferriswheel:  { name:'페리스휠', color:'#EBBE92', rgb:'235,190,146', solid:'#E7B07A', solidRgb:'231,176,122' },
    lilacblue2:  { name:'라일락블루 2', color:'#869EDD', rgb:'134,158,221', solid:'#6C89D6', solidRgb:'108,137,214' },
    ivywoods2:  { name:'아이비우즈 2', color:'#C5CDBC', rgb:'197,205,188', solid:'#B8C2AD', solidRgb:'184,194,173' },
    dreamysky:  { name:'드리미스카이', color:'#D3BDEB', rgb:'211,189,235', solid:'#C9AEE6', solidRgb:'201,174,230' },
    vintagesky2:  { name:'빈티지스카이 2', color:'#535353', rgb:'83,83,83', solid:'#2D2D2D', solidRgb:'45,45,45' },
    dahliadahlia:  { name:'달리아달리아', color:'#B8D36B', rgb:'184,211,107', solid:'#A8C94A', solidRgb:'168,201,74' },
    goldengreenery:  { name:'골든그리너리', color:'#86B566', rgb:'134,181,102', solid:'#6BA544', solidRgb:'107,165,68' },
    peach:  { name:'피치', color:'#F7B893', rgb:'247,184,147', solid:'#F5A97B', solidRgb:'245,169,123' },
    summerocean2:  { name:'서머오션 2', color:'#4EC3E7', rgb:'78,195,231', solid:'#27B6E2', solidRgb:'39,182,226' },
    christmas2:  { name:'크리스마스 2', color:'#C35D5D', rgb:'195,93,93', solid:'#B63A3A', solidRgb:'182,58,58' },
    deepwave2:  { name:'딥웨이브 2', color:'#47618F', rgb:'71,97,143', solid:'#1F3E77', solidRgb:'31,62,119' },
};
...
const SUBJECT_SWATCHES = {
    ...
    barbiedoll:  { 국어:'#DD5F9B', 영어:'#DD765F', 행정학:'#DDC95F', 행정법:'#5FDD65', 단어:'#5FDDC7', 모의고사:'#5F73DD' },
    peachfizz:  { 국어:'#DD7A5F', 영어:'#DDCE5F', 행정학:'#99DD5F', 행정법:'#5FDDBC', 단어:'#5F9CDD', 모의고사:'#A35FDD' },
    summertime:  { 국어:'#5FD1DD', 영어:'#5F7EDD', 행정학:'#945FDD', 행정법:'#DD5F8F', 단어:'#DD915F', 모의고사:'#A8DD5F' },
    macaronforest:  { 국어:'#7CC094', 영어:'#7CBFC0', 행정학:'#7C92C0', 행정법:'#B77CC0', 단어:'#C07C94', 모의고사:'#C0AA7C' },
    pinkrose:  { 국어:'#CE6F8E', 영어:'#CE8E6F', 행정학:'#CECC6F', 행정법:'#6FCE80', 단어:'#6FCECA', 모의고사:'#6F70CE' },
    cafelatte:  { 국어:'#C59978', 영어:'#BEC578', 행정학:'#8BC578', 행정법:'#78C5C1', 단어:'#788CC5', 모의고사:'#B178C5' },
    vintagesky:  { 국어:'#7C9BC0', 영어:'#8B7CC0', 행정학:'#B77CC0', 행정법:'#C0817C', 단어:'#C0B67C', 모의고사:'#85C07C' },
    taiga:  { 국어:'#7CA0C0', 영어:'#857CC0', 행정학:'#B27CC0', 행정법:'#C07C7C', 단어:'#C0B17C', 모의고사:'#8AC07C' },
    cupcake:  { 국어:'#6ECEBC', 영어:'#6EA1CE', 행정학:'#7A6ECE', 행정법:'#CE6EAF', 단어:'#CE786E', 모의고사:'#C2CE6E' },
    summerocean:  { 국어:'#60DADC', 영어:'#6088DC', 행정학:'#8A60DC', 행정법:'#DC6099', 단어:'#DC8860', 모의고사:'#B2DC60' },
    oceandeep:  { 국어:'#6D8BCF', 영어:'#906DCF', 행정학:'#CF6DCE', 행정법:'#CF826D', 단어:'#CFCF6D', 모의고사:'#6DCF6F' },
    cookiencream:  { 국어:'#C09E7C', 영어:'#B5C07C', 행정학:'#88C07C', 행정법:'#7CBFC0', 단어:'#7C8AC0', 모의고사:'#B47CC0' },
    hellorustone:  { 국어:'#7C99C0', 영어:'#8C7CC0', 행정학:'#B97CC0', 행정법:'#C0837C', 단어:'#C0B87C', 모의고사:'#83C07C' },
    amethyst:  { 국어:'#9E7CC0', 영어:'#C07CB6', 행정학:'#C07C89', 행정법:'#BFC07C', 단어:'#8AC07C', 모의고사:'#7CC0B3' },
    heliotrope:  { 국어:'#6F97CD', 영어:'#856FCD', 행정학:'#C36FCD', 행정법:'#CD786F', 단어:'#CDC16F', 모의고사:'#79CD6F' },
    christmas:  { 국어:'#D26A6A', 영어:'#D2AF6A', 행정학:'#B1D26A', 행정법:'#6AD2A0', 단어:'#6AB3D2', 모의고사:'#8B6AD2' },
    dahlia:  { 국어:'#B6C479', 영어:'#84C479', 행정학:'#79C49E', 행정법:'#798FC4', 단어:'#9D79C4', 모의고사:'#C4799E' },
    /* ── 컬러표 이미지 원본색(6번째 '기타' 칸은 모의고사·기타 과목에 함께 사용) ── */
    hellolisbon:  { 국어:'#E8B7BD', 영어:'#C7DEEF', 행정학:'#AEBFB9', 행정법:'#C6B5D6', 단어:'#E9E0C1', 모의고사:'#DAB8B2', 기타:'#DAB8B2' },
    summertime2:  { 국어:'#F2B8D3', 영어:'#70CCDA', 행정학:'#D6F45B', 행정법:'#809DE1', 단어:'#F4C934', 모의고사:'#EB99BC', 기타:'#EB99BC' },
    ivywoods:  { 국어:'#B2C7C9', 영어:'#C4BFAF', 행정학:'#677766', 행정법:'#BCCAAD', 단어:'#E6E4CD', 모의고사:'#697563', 기타:'#697563' },
    veryperi2:  { 국어:'#8E94C0', 영어:'#7C9B97', 행정학:'#C7979D', 행정법:'#744B5B', 단어:'#91A7CA', 모의고사:'#C3B584', 기타:'#C3B584' },
    meganisi:  { 국어:'#36595E', 영어:'#DBC2B1', 행정학:'#6C7D74', 행정법:'#D8B5A1', 단어:'#D7BB9C', 모의고사:'#B19A78', 기타:'#B19A78' },
    denimlove:  { 국어:'#D293A0', 영어:'#EBE7D0', 행정학:'#E8A97D', 행정법:'#AC7DA9', 단어:'#E2DA8B', 모의고사:'#EBB892', 기타:'#EBB892' },
    sereneearth:  { 국어:'#858F76', 영어:'#B5B993', 행정학:'#96A1B6', 행정법:'#677083', 단어:'#91806D', 모의고사:'#CDBEA2', 기타:'#CDBEA2' },
    cozycity:  { 국어:'#9DA697', 영어:'#A2B5C5', 행정학:'#B5AE5E', 행정법:'#C6AB9D', 단어:'#DBD8BC', 모의고사:'#687262', 기타:'#687262' },
    dreamysurf2:  { 국어:'#F2DCE4', 영어:'#8E9EAF', 행정학:'#E6CFD3', 행정법:'#ECE3DE', 단어:'#D9C9C3', 모의고사:'#CDE8EC', 기타:'#CDE8EC' },
    grapefruit:  { 국어:'#F5B18B', 영어:'#EF899D', 행정학:'#EE6F6D', 행정법:'#F8CBAB', 단어:'#F8A9BC', 모의고사:'#F16F4D', 기타:'#F16F4D' },
    /* ── 컬러표 2차: 이미지 원본색 ── */
    mapleleaves:  { 국어:'#EF8A82', 영어:'#6A8F86', 행정학:'#F07D3C', 행정법:'#D7C8A6', 단어:'#4B2E24', 모의고사:'#6F8A5B', 기타:'#6F8A5B' },
    rosetone:  { 국어:'#E39AA7', 영어:'#A7B9C6', 행정학:'#B86574', 행정법:'#7D9BAF', 단어:'#F2D8DE', 모의고사:'#E9ECE3', 기타:'#E9ECE3' },
    taiga2:  { 국어:'#F5D6E1', 영어:'#B8D7E6', 행정학:'#A8CFC7', 행정법:'#C9B9D9', 단어:'#EFE6D6', 모의고사:'#EAF3F7', 기타:'#EAF3F7' },
    pebble2:  { 국어:'#8FA6B1', 영어:'#9ECAD3', 행정학:'#CDD6DC', 행정법:'#3C4A51', 단어:'#B7B7B3', 모의고사:'#EFEEEB', 기타:'#EFEEEB' },
    ferriswheel:  { 국어:'#F19B72', 영어:'#CFE3D9', 행정학:'#F07D4D', 행정법:'#A8C37C', 단어:'#E6D5AE', 모의고사:'#E2C76A', 기타:'#E2C76A' },
    lilacblue2:  { 국어:'#9FB8E5', 영어:'#E7ECF4', 행정학:'#5D72B0', 행정법:'#3F5E9A', 단어:'#A6B4D1', 모의고사:'#E7ECF4', 기타:'#E7ECF4' },
    ivywoods2:  { 국어:'#DAD7C2', 영어:'#6A7E61', 행정학:'#A6B48E', 행정법:'#8FA17A', 단어:'#9A9589', 모의고사:'#C6C0B4', 기타:'#C6C0B4' },
    dreamysky:  { 국어:'#E9A3CF', 영어:'#BDA5DF', 행정학:'#A97DBF', 행정법:'#8F95D6', 단어:'#F4E7E1', 모의고사:'#C9D3DE', 기타:'#C9D3DE' },
    vintagesky2:  { 국어:'#5F6366', 영어:'#8EA0A6', 행정학:'#7E8479', 행정법:'#A19D95', 단어:'#D5CBB8', 모의고사:'#E7E6E3', 기타:'#E7E6E3' },
    dahliadahlia:  { 국어:'#C8E78A', 영어:'#6F8E3B', 행정학:'#7E3D9A', 행정법:'#5B2D8A', 단어:'#C9A7D8', 모의고사:'#D84C8D', 기타:'#D84C8D' },
    goldengreenery:  { 국어:'#D7E8A6', 영어:'#3F6F2F', 행정학:'#B8D65C', 행정법:'#E6F0C1', 단어:'#F5F9EC', 모의고사:'#C8E7B2', 기타:'#C8E7B2' },
    peach:  { 국어:'#FFB696', 영어:'#8CA67A', 행정학:'#C49A6C', 행정법:'#E2C9AE', 단어:'#FDE8D6', 모의고사:'#F8D1C0', 기타:'#F8D1C0' },
    summerocean2:  { 국어:'#8EDFF1', 영어:'#0EA0B8', 행정학:'#3BC6D4', 행정법:'#0C78A8', 단어:'#C6EDF5', 모의고사:'#075B74', 기타:'#075B74' },
    christmas2:  { 국어:'#E57373', 영어:'#2E6B3F', 행정학:'#9BC88B', 행정법:'#E7C9A6', 단어:'#8C5E3C', 모의고사:'#4E6B43', 기타:'#4E6B43' },
    deepwave2:  { 국어:'#3A64C8', 영어:'#8EB8E6', 행정학:'#2A7F9E', 행정법:'#0E2A56', 단어:'#7FA6E5', 모의고사:'#4DB6C4', 기타:'#4DB6C4' },
};
...
const SUBJECT_BG = {
    ...
    barbiedoll:  { 국어:'#E3C0D0', 영어:'#E3C6C0', 행정학:'#E3DDC0', 행정법:'#C0E3C1', 단어:'#C0E3DC', 모의고사:'#C0C5E3' },
    peachfizz:  { 국어:'#E3C7C0', 영어:'#E3DEC0', 행정학:'#D0E3C0', 행정법:'#C0E3D9', 단어:'#C0D0E3', 모의고사:'#D2C0E3' },
    summertime:  { 국어:'#C0DFE3', 영어:'#C0C8E3', 행정학:'#CEC0E3', 행정법:'#E3C0CD', 단어:'#E3CEC0', 모의고사:'#D4E3C0' },
    macaronforest:  { 국어:'#C0E3CC', 영어:'#C0E2E3', 행정학:'#C0CBE3', 행정법:'#DEC0E3', 단어:'#E3C0CC', 모의고사:'#E3D7C0' },
    pinkrose:  { 국어:'#E3C0CB', 영어:'#E3CBC0', 행정학:'#E3E2C0', 행정법:'#C0E3C6', 단어:'#C0E3E1', 모의고사:'#C0C0E3' },
    cafelatte:  { 국어:'#E3CFC0', 영어:'#DFE3C0', 행정학:'#C8E3C0', 행정법:'#C0E3E1', 단어:'#C0C9E3', 모의고사:'#DAC0E3' },
    vintagesky:  { 국어:'#C0CFE3', 영어:'#C7C0E3', 행정학:'#DEC0E3', 행정법:'#E3C2C0', 단어:'#E3DDC0', 모의고사:'#C4E3C0' },
    taiga:  { 국어:'#C0D2E3', 영어:'#C4C0E3', 행정학:'#DBC0E3', 행정법:'#E3C0C0', 단어:'#E3DBC0', 모의고사:'#C7E3C0' },
    cupcake:  { 국어:'#C0E3DC', 영어:'#C0D2E3', 행정학:'#C4C0E3', 행정법:'#E3C0D7', 단어:'#E3C3C0', 모의고사:'#DEE3C0' },
    summerocean:  { 국어:'#C0E2E3', 영어:'#C0CBE3', 행정학:'#CBC0E3', 행정법:'#E3C0D0', 단어:'#E3CBC0', 모의고사:'#D7E3C0' },
    oceandeep:  { 국어:'#C0CAE3', 영어:'#CCC0E3', 행정학:'#E3C0E2', 행정법:'#E3C7C0', 단어:'#E3E2C0', 모의고사:'#C0E3C0' },
    cookiencream:  { 국어:'#E3D1C0', 영어:'#DDE3C0', 행정학:'#C6E3C0', 행정법:'#C0E2E3', 단어:'#C0C7E3', 모의고사:'#DCC0E3' },
    hellorustone:  { 국어:'#C0CEE3', 영어:'#C8C0E3', 행정학:'#DFC0E3', 행정법:'#E3C3C0', 단어:'#E3DEC0', 모의고사:'#C3E3C0' },
    amethyst:  { 국어:'#D1C0E3', 영어:'#E3C0DD', 행정학:'#E3C0C6', 행정법:'#E2E3C0', 단어:'#C7E3C0', 모의고사:'#C0E3DC' },
    heliotrope:  { 국어:'#C0CFE3', 영어:'#C8C0E3', 행정학:'#DFC0E3', 행정법:'#E3C3C0', 단어:'#E3DEC0', 모의고사:'#C3E3C0' },
    christmas:  { 국어:'#E3C0C0', 영어:'#E3D7C0', 행정학:'#D7E3C0', 행정법:'#C0E3D2', 단어:'#C0D8E3', 모의고사:'#CBC0E3' },
    dahlia:  { 국어:'#DCE3C0', 영어:'#C5E3C0', 행정학:'#C0E3D1', 행정법:'#C0CAE3', 단어:'#D0C0E3', 모의고사:'#E3C0D1' },
    /* ── 컬러표 색상을 배지용으로 가공(색상 유지, 밝기·채도 보정, 너무 비슷한 쌍만 최소한으로 벌림) ── */
    hellolisbon:  { 국어:'#EFCAD3', 영어:'#D7E7F4', 행정학:'#D3DEDA', 행정법:'#DED4E7', 단어:'#F0EAD6', 모의고사:'#EAD6D2', 기타:'#EAD6D2' },
    summertime2:  { 국어:'#F4D2E2', 영어:'#B9E6ED', 행정학:'#E3EEB9', 행정법:'#BECDEF', 단어:'#EBDDAF', 모의고사:'#EEBCD2', 기타:'#EEBCD2' },
    ivywoods:  { 국어:'#D5E0E1', 영어:'#DFDDD4', 행정학:'#B2D1AE', 행정법:'#D4DCC9', 단어:'#EFEDDA', 모의고사:'#BCC9B5', 기타:'#BCC9B5' },
    veryperi2:  { 국어:'#C0C1DC', 영어:'#C2D2D0', 행정학:'#E2CACD', 행정법:'#C9ACB8', 단어:'#C7D2E4', 모의고사:'#E2DBC3', 기타:'#E2DBC3' },
    meganisi:  { 국어:'#9FC3C8', 영어:'#F2E8E3', 행정학:'#B9CBC1', 행정법:'#E4C4BE', 단어:'#EADACA', 모의고사:'#D4C9B6', 기타:'#D4C9B6' },
    denimlove:  { 국어:'#E8C7CE', 영어:'#F0EDDC', 행정학:'#EDC8B7', 행정법:'#D8C2D7', 단어:'#F0EBC2', 모의고사:'#F1D8C6', 기타:'#F1D8C6' },
    sereneearth:  { 국어:'#C8CFBE', 영어:'#DCDFC7', 행정학:'#CBD1DB', 행정법:'#B9BFCB', 단어:'#CEC6BD', 모의고사:'#E4DCCE', 기타:'#E4DCCE' },
    cozycity:  { 국어:'#CFD7C9', 영어:'#CED8E1', 행정학:'#DDDAB5', 행정법:'#E2D4CC', 단어:'#E9E7D6', 모의고사:'#BCC8B5', 기타:'#BCC8B5' },
    dreamysurf2:  { 국어:'#EBC9D6', 영어:'#C8D0D8', 행정학:'#EDDDE0', 행정법:'#EFE9E3', 단어:'#DCCDC7', 모의고사:'#DAEEF1', 기타:'#DAEEF1' },
    grapefruit:  { 국어:'#F1D6C7', 영어:'#F0C5CD', 행정학:'#EBB1B1', 행정법:'#F6E7DC', 단어:'#F6DCE2', 모의고사:'#ECC0B5', 기타:'#ECC0B5' },
    /* ── 컬러표 2차: 배지용 파스텔 ── */
    mapleleaves:  { 국어:'#F0C6C3', 영어:'#BCCDC9', 행정학:'#EBC5B0', 행정법:'#E9E1CE', 단어:'#CDA598', 모의고사:'#BFCEB5', 기타:'#BFCEB5' },
    rosetone:  { 국어:'#F0C7CE', 영어:'#D0DAE1', 행정학:'#D9AEB6', 행정법:'#B9C9D3', 단어:'#F4DDE3', 모의고사:'#EBEEE5', 기타:'#EBEEE5' },
    taiga2:  { 국어:'#F6DBE5', 영어:'#CCDEEC', 행정학:'#D0E5E1', 행정법:'#DFD6E8', 단어:'#F2EBDE', 모의고사:'#E4F0F5', 기타:'#E4F0F5' },
    pebble2:  { 국어:'#C3CDD5', 영어:'#CBE3E7', 행정학:'#DEE3E8', 행정법:'#A7B6BE', 단어:'#DEDED2', 모의고사:'#EEEDE8', 기타:'#EEEDE8' },
    ferriswheel:  { 국어:'#EFCEBF', 영어:'#DEEBE4', 행정학:'#EAB8AC', 행정법:'#D5E2C0', 단어:'#F0E6CF', 모의고사:'#EDE2B9', 기타:'#EDE2B9' },
    lilacblue2:  { 국어:'#C9D3F0', 영어:'#D9E0EE', 행정학:'#B5BFDB', 행정법:'#9DABD7', 단어:'#C2D0DF', 모의고사:'#E4EFF3', 기타:'#E4EFF3' },
    ivywoods2:  { 국어:'#ECE9D5', 영어:'#B5C4AE', 행정학:'#D5E0C3', 행정법:'#CBD4C2', 단어:'#D0C8BE', 모의고사:'#E0DDD5', 기타:'#E0DDD5' },
    dreamysky:  { 국어:'#F2CAE3', 영어:'#E2D7F1', 행정학:'#D6C0E0', 행정법:'#C5C8EA', 단어:'#F4E7E1', 모의고사:'#DCE2E9', 기타:'#DCE2E9' },
    vintagesky2:  { 국어:'#B2BDC6', 영어:'#C8D2D6', 행정학:'#C5CEBD', 행정법:'#D6D2C8', 단어:'#EBE2D2', 모의고사:'#ECEAE5', 기타:'#ECEAE5' },
    dahliadahlia:  { 국어:'#E1F0C3', 영어:'#C3D8A1', 행정학:'#CAA3DB', 행정법:'#B996DD', 단어:'#E1CEE9', 모의고사:'#EBAECA', 기타:'#EBAECA' },
    goldengreenery:  { 국어:'#E9F0C3', 영어:'#A6D397', 행정학:'#DDECB2', 행정법:'#EEF4D4', 단어:'#F1F6E4', 모의고사:'#D4EEC8', 기타:'#D4EEC8' },
    peach:  { 국어:'#F0CDC5', 영어:'#CAD6C1', 행정학:'#DFC8AE', 행정법:'#EBDEC7', 단어:'#F6EEDF', 모의고사:'#F5E0D6', 기타:'#F5E0D6' },
    summerocean2:  { 국어:'#BFE1EF', 영어:'#94D9E4', 행정학:'#B0EBEB', 행정법:'#87BCE1', 단어:'#D7F0F5', 모의고사:'#85CBE0', 기타:'#85CBE0' },
    christmas2:  { 국어:'#EEBCBC', 영어:'#96D2A7', 행정학:'#CDE3C5', 행정법:'#F1E0CB', 단어:'#D7B8A2', 모의고사:'#B0C8A8', 기타:'#B0C8A8' },
    deepwave2:  { 국어:'#A6B9E6', 영어:'#CCE2F2', 행정학:'#96CEE3', 행정법:'#85A8E0', 단어:'#BFD1EF', 모의고사:'#AEDEE4', 기타:'#AEDEE4' },
};
...
/* ═══════════════════════════════════════════════
   테마 커스터마이징 — 기본 테마 색 조정 / 나만의 테마 만들기 / 삭제·복원
   - 기본 테마는 코드의 원본(THEMES_ORIG·SUBJECT_BG_ORIG)을 그대로 두고, 바꾼 값만 localStorage에 조정값으로 저장한다.
       young_theme_overrides : { 테마키: { solid?: '#대표색', subj?: { 과목: '#배경색', ... } } }   (바꾼 것만 저장)
   - 나만의 테마는 전체 색을 localStorage에 저장한다.
       young_custom_themes   : { custom_xxx: { name, emoji, solid, subj:{11과목} } }
   - '원래대로'는 조정값만 지우면 코드의 원본 색으로 돌아간다.
═══════════════════════════════════════════════ */
const THEMES_ORIG = JSON.parse(JSON.stringify(THEMES));
const SUBJECT_BG_ORIG = JSON.parse(JSON.stringify(SUBJECT_BG));
const THEME_OVERRIDES_KEY = 'young_theme_overrides';
const CUSTOM_THEMES_KEY = 'young_custom_themes';
const THEME_EDIT_SUBJECTS_MAIN = ['국어','영어','행정학','행정법','단어','모의고사'];
const THEME_EDIT_SUBJECTS_MORE = ['한국사','헌법','경제','각론','기타'];
const THEME_EDIT_SUBJECTS = [...THEME_EDIT_SUBJECTS_MAIN, ...THEME_EDIT_SUBJECTS_MORE];

function loadJsonObj(key){
    try{ const v=JSON.parse(localStorage.getItem(key)); return (v && typeof v==='object' && !Array.isArray(v)) ? v : {}; }
    catch{ return {}; }
}
let themeOverrides = loadJsonObj(THEME_OVERRIDES_KEY);
let customThemes   = loadJsonObj(CUSTOM_THEMES_KEY);
function saveThemeOverrides(){ try{ localStorage.setItem(THEME_OVERRIDES_KEY, JSON.stringify(themeOverrides)); }catch{} }
function saveCustomThemes(){ try{ localStorage.setItem(CUSTOM_THEMES_KEY, JSON.stringify(customThemes)); }catch{} }

const _isHex6 = (h)=>/^#[0-9a-fA-F]{6}$/.test(h||'');
const _rgbStr = (hex)=>{ const v=hex.replace('#',''); return [0,2,4].map(i=>parseInt(v.slice(i,i+2),16)).join(','); };
function escThemeText(s){ return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
// 대표색(solid)에서 연한 강조색(color = 흰색 18% 혼합)과 rgb 문자열을 만든다
function makeThemeColors(solid){
    solid = solid.toUpperCase();
    const color = mixHexWithWhite(solid, .18).toUpperCase();
    return { color, rgb:_rgbStr(color), solid, solidRgb:_rgbStr(solid) };
}
// 과목 배지 배경은 어두우면 (배지 글씨가 항상 어두운 색이라) 안 읽히므로 명도 하한(.62)만 보정한다
function normalizeSubjectBg(hex){
    if(!_isHex6(hex)) return hex;
    const [h,s,l] = hexToHsl(hex);
    if(l >= .62) return hex.toUpperCase();
    return hslToHex(h, s, .62).toUpperCase();
}

// 코드의 원본 + 조정값 + 내 테마를 합쳐서 THEMES / SUBJECT_BG를 만든다(여러 번 불러도 결과가 같다)
function applyThemeCustomizations(){
    Object.keys(THEMES_ORIG).forEach(k=>{
        THEMES[k] = JSON.parse(JSON.stringify(THEMES_ORIG[k]));
        if(SUBJECT_BG_ORIG[k]) SUBJECT_BG[k] = JSON.parse(JSON.stringify(SUBJECT_BG_ORIG[k]));
        const ov = themeOverrides[k];
        if(!ov) return;
        if(_isHex6(ov.solid)) Object.assign(THEMES[k], makeThemeColors(ov.solid));
        if(ov.subj){
            if(!SUBJECT_BG[k]) SUBJECT_BG[k] = {};
            Object.entries(ov.subj).forEach(([s,c])=>{ if(_isHex6(c)) SUBJECT_BG[k][s] = normalizeSubjectBg(c); });
        }
    });
    Object.keys(THEMES).forEach(k=>{ if(THEMES[k].custom){ delete THEMES[k]; delete SUBJECT_BG[k]; } });
    Object.entries(customThemes).forEach(([k,ct])=>{
        if(!ct || !_isHex6(ct.solid)) return;
        THEMES[k] = { name:String(ct.name||'내 테마').slice(0,20), emoji:ct.emoji||'🎨', custom:true, ...makeThemeColors(ct.solid) };
        SUBJECT_BG[k] = {};
        THEME_EDIT_SUBJECTS.forEach(s=>{ SUBJECT_BG[k][s] = _isHex6(ct.subj?.[s]) ? normalizeSubjectBg(ct.subj[s]) : '#E5E7EB'; });
    });
}
applyThemeCustomizations();

/* ═══════════════════════════════════════════════
   테마 팔레트: THEMES에서 자동 생성 + 무지개(색상) 순 자동 정렬 + 테마 삭제(숨김)/복원
   - 새 테마는 THEMES에 추가만 하면 팔레트에 자동으로 알맞은 자리에 들어간다(내 테마도 마찬가지).
   - 기본 테마 '삭제'는 팔레트에서 숨기는 것(복원 가능), 내 테마 '삭제'는 완전 삭제다.
   - 삭제 목록은 localStorage('young_hidden_themes')에 저장된다.
═══════════════════════════════════════════════ */
const THEME_HIDDEN_KEY = 'young_hidden_themes';
// 예전 팔레트에 원래 버튼이 없었던 테마는 처음부터 숨김 상태로 시작(그대로 유지). 관리 창에서 복원 가능
const THEME_DEFAULT_HIDDEN = ['dreamysurf'];
let hiddenThemes = (()=>{
    try{
        const raw = localStorage.getItem(THEME_HIDDEN_KEY);
        if(raw===null) return THEME_DEFAULT_HIDDEN.filter(k=>THEMES[k]);
        const arr = JSON.parse(raw);
        return Array.isArray(arr) ? arr.filter(k=>THEMES[k]) : [];
    }catch{ return THEME_DEFAULT_HIDDEN.filter(k=>THEMES[k]); }
})();
function saveHiddenThemes(){ try{ localStorage.setItem(THEME_HIDDEN_KEY, JSON.stringify(hiddenThemes)); }catch{} }

// 동그라미 색(solid) 기준 정렬: 빨강→주황→노랑→초록→청록·파랑→남색→보라→분홍, 채도가 낮은 회색톤은 맨 끝(어두운→밝은)
function getSortedThemeKeys(){
    const NEUTRAL_S = 0.20;  // 이 채도보다 낮으면 회색톤
    const RED_START = 345;   // 이 색상각부터 '빨강' 시작(그 앞쪽 분홍~마젠타는 보라 뒤로)
    const info = Object.keys(THEMES).map(k=>{
        const [h,s,l] = hexToHsl(THEMES[k].solid || THEMES[k].color);
        return {k,h,s,l};
    });
    const rot = (h)=>((h-RED_START)%360+360)%360;
    const chrom = info.filter(x=>x.s>=NEUTRAL_S).sort((a,b)=>(rot(a.h)-rot(b.h)) || (a.l-b.l));
    const neut  = info.filter(x=>x.s<NEUTRAL_S).sort((a,b)=>a.l-b.l);
    return [...chrom, ...neut].map(x=>x.k);
}
// 지금 팔레트에 보이는 테마(사용 중인 테마는 숨김 목록에 있어도 항상 보임)
function getVisibleThemeKeys(){
    return getSortedThemeKeys().filter(k=>!hiddenThemes.includes(k) || k===currentTheme);
}
function renderThemePalette(){
    const pal = document.getElementById('theme-palette');
    if(!pal) return;
    pal.innerHTML = getVisibleThemeKeys().map(k=>{
        const t = THEMES[k];
        return `<button class="w-4 h-4 rounded-full" onclick="setTheme('${k}')" style="background:${t.solid}" title="${escThemeText(t.name)}"></button>`;
    }).join('');
}

/* ── 색 조정 편집기 ── */
let themeDraft = null; // { mode:'builtin'|'custom'|'create', key, name, emoji, solid, subj:{과목:'#색'} }
function effectiveSubjectBgFrom(themesTbl, bgTbl, key, subj){
    const t = bgTbl[key];
    if(t && t[subj]) return t[subj];
    const base = themesTbl[key]?.solid || themesTbl[key]?.color || themesTbl.clearsky.color;
    const tint = TIMELINE_SUBJECT_TINTS[subj] ?? TIMELINE_SUBJECT_TINTS['기타'];
    return paletteColor(mixHexWithWhite(base, tint), .28, .72, .88, .50);
}
const effectiveSubjectBg = (k,s)=>effectiveSubjectBgFrom(THEMES, SUBJECT_BG, k, s).toUpperCase();
const originalSubjectBg  = (k,s)=>effectiveSubjectBgFrom(THEMES_ORIG, SUBJECT_BG_ORIG, k, s).toUpperCase();

function openThemeEditor(key){
    const t = THEMES[key];
    if(!t) return;
    themeDraft = { mode: t.custom?'custom':'builtin', key, name:t.name, emoji:t.emoji||'', solid:t.solid.toUpperCase(), subj:{} };
    THEME_EDIT_SUBJECTS.forEach(s=>{ themeDraft.subj[s] = effectiveSubjectBg(key, s); });
    showThemeEditor();
}
function openThemeCreator(baseKey){
    const bk = THEMES[baseKey] ? baseKey : (THEMES[currentTheme] ? currentTheme : 'clearsky');
    themeDraft = { mode:'create', key:null, name:'내 테마', emoji:'🎨', solid:THEMES[bk].solid.toUpperCase(), subj:{} };
    THEME_EDIT_SUBJECTS.forEach(s=>{ themeDraft.subj[s] = effectiveSubjectBg(bk, s); });
    showThemeEditor();
}
function cloneThemeFromEditor(){
    const d = themeDraft;
    if(!d) return;
    themeDraft = { mode:'create', key:null, name:(String(d.name||'내 테마')+' 복사본').slice(0,20), emoji:d.emoji||'🎨', solid:d.solid, subj:{...d.subj} };
    showThemeEditor();
}
function buildThemeEditorHtml(){
    const d = themeDraft;
    if(!d) return '';
    const colorInput = (hex, handler)=>`<input type="color" value="${hex.toLowerCase()}" oninput="${handler}" style="width:46px;height:34px;padding:0;border:1px solid #e2e8f0;border-radius:8px;background:#fff;flex-shrink:0;">`;
    const subjRow = (s)=>{
        const bg = normalizeSubjectBg(d.subj[s]);
        const tx = paletteTextOn(bg);
        return `<div class="flex items-center gap-2.5 py-1">
            <span id="ed-prev-${s}" class="text-[13px] font-black px-3 py-1.5 rounded-full flex-1 text-center" style="background:${bg};color:${tx};border:1px solid ${hexToRgba(tx,.35)}">${s}</span>
            ${colorInput(d.subj[s], `editorSetSubj('${s}',this.value)`)}
        </div>`;
    };
    const isCreate = d.mode==='create', isCustom = d.mode==='custom', isBuiltin = d.mode==='builtin';
    const title = isCreate ? '새 테마 만들기' : `${escThemeText(d.name)} 색 조정`;
    const nameRow = (isCreate||isCustom) ? `<div class="flex items-center gap-2 mb-3">
            <input type="text" value="${escThemeText(d.emoji||'')}" maxlength="4" placeholder="🎨" oninput="editorSetEmoji(this.value)" style="width:52px;text-align:center;font-size:18px;padding:6px 4px;border:1px solid #e2e8f0;border-radius:10px;">
            <input type="text" value="${escThemeText(d.name)}" maxlength="20" placeholder="테마 이름" oninput="editorSetName(this.value)" class="flex-1 min-w-0 text-[14px] font-black text-slate-700" style="padding:7px 10px;border:1px solid #e2e8f0;border-radius:10px;">
        </div>` : '';
    const extra = [
        isBuiltin && themeOverrides[d.key] ? `<button onclick="resetThemeOverrides('${d.key}')" class="w-full text-[12px] font-black py-2.5 rounded-xl text-slate-500" style="background:#f8fafc;border:1px solid #eef2f6;">↩︎ 이 테마 색 원래대로</button>` : '',
        !isCreate ? `<button onclick="cloneThemeFromEditor()" class="w-full text-[12px] font-black py-2.5 rounded-xl text-slate-500" style="background:#f8fafc;border:1px solid #eef2f6;">🧬 이 색으로 복제해서 새 테마 만들기</button>` : '',
        isCustom ? `<button onclick="deleteCustomTheme('${d.key}')" class="w-full text-[12px] font-black py-2.5 rounded-xl" style="background:#fff5f5;border:1px solid #fee2e2;color:#ef4444;">🗑 이 테마 삭제</button>` : '',
    ].filter(Boolean).join('');
    return `<div style="width:100%;max-width:520px;max-height:88vh;overflow:auto;background:#fff;border-radius:22px 22px 0 0;padding:18px 16px calc(24px + env(safe-area-inset-bottom, 0px));box-shadow:0 -10px 40px rgba(15,23,42,.2);">
        <div class="flex items-center justify-between mb-3">
            <div class="text-[16px] font-black text-slate-800">${title}</div>
            <button onclick="closeThemeEditor()" aria-label="닫기" class="text-[22px] leading-none font-black text-slate-400 px-2">×</button>
        </div>
        ${nameRow}
        <div class="flex items-center gap-2.5 mb-4 rounded-xl px-3 py-2.5" style="background:#f8fafc;border:1px solid #eef2f6;">
            <span id="ed-solid-prev" style="width:26px;height:26px;border-radius:9999px;background:${d.solid};flex-shrink:0;box-shadow:inset 0 0 0 1px rgba(15,23,42,.1);"></span>
            <div class="flex-1 min-w-0">
                <div class="text-[13px] font-black text-slate-700">대표색</div>
                <div class="text-[10px] font-bold text-slate-400">팔레트 동그라미·버튼·강조 글씨 색</div>
            </div>
            ${colorInput(d.solid, `editorSetSolid(this.value)`)}
        </div>
        <div class="text-[12px] font-black text-slate-500 mb-1">과목 색 <span class="text-slate-400 font-bold">— 배지·타임라인·기록에 쓰여요</span></div>
        ${THEME_EDIT_SUBJECTS_MAIN.map(subjRow).join('')}
        <div class="text-[12px] font-black text-slate-500 mt-3 mb-1">그 외 과목 ${isBuiltin?'<span class="text-slate-400 font-bold">— 바꾸지 않으면 대표색에서 자동으로 만들어져요</span>':''}</div>
        ${THEME_EDIT_SUBJECTS_MORE.map(subjRow).join('')}
        <div class="text-[10px] font-bold text-slate-400 mt-2">너무 어두운 색은 글씨가 안 보여서 자동으로 살짝 밝게 저장돼요.</div>
        <div class="flex gap-2 mt-4">
            <button onclick="saveThemeEditor()" class="flex-1 py-3 rounded-xl text-[14px] font-black text-white" style="background:var(--pc-solid)">${isCreate?'테마 만들기':'저장'}</button>
            <button onclick="closeThemeEditor()" class="px-5 py-3 rounded-xl text-[14px] font-black text-slate-500" style="background:#f1f5f9">취소</button>
        </div>
        ${extra ? `<div class="space-y-2 mt-3">${extra}</div>` : ''}
    </div>`;
}
function showThemeEditor(){
    let ov = document.getElementById('theme-editor');
    if(!ov){
        ov = document.createElement('div');
        ov.id = 'theme-editor';
        ov.style.cssText = 'position:fixed;inset:0;z-index:210;background:rgba(15,23,42,.5);display:none;align-items:flex-end;justify-content:center;';
        ov.onclick = (e)=>{ if(e.target===ov) closeThemeEditor(); };
        document.body.appendChild(ov);
    }
    ov.innerHTML = buildThemeEditorHtml();
    ov.style.display = 'flex';
}
function closeThemeEditor(){
    const ov = document.getElementById('theme-editor');
    if(ov) ov.style.display = 'none';
    themeDraft = null;
}
function editorSetName(v){ if(themeDraft) themeDraft.name = v; }
function editorSetEmoji(v){ if(themeDraft) themeDraft.emoji = v; }
function editorSetSolid(hex){
    if(!themeDraft || !_isHex6(hex)) return;
    themeDraft.solid = hex.toUpperCase();
    const el = document.getElementById('ed-solid-prev');
    if(el) el.style.background = hex;
}
function editorSetSubj(s, hex){
    if(!themeDraft || !_isHex6(hex)) return;
    themeDraft.subj[s] = hex.toUpperCase();
    const bg = normalizeSubjectBg(hex), tx = paletteTextOn(bg);
    const el = document.getElementById('ed-prev-'+s);
    if(el){ el.style.background = bg; el.style.color = tx; el.style.borderColor = hexToRgba(tx,.35); }
}
function saveThemeEditor(){
    const d = themeDraft;
    if(!d) return;
    const name  = (String(d.name||'').trim() || '내 테마').slice(0,20);
    const emoji = (String(d.emoji||'').trim() || '🎨').slice(0,4);
    const subj = {};
    THEME_EDIT_SUBJECTS.forEach(s=>{ subj[s] = normalizeSubjectBg(d.subj[s]); });
    let msg = '색을 저장했어요';
    if(d.mode==='create'){
        let key = 'custom_' + Date.now().toString(36);
        while(customThemes[key] || THEMES[key]) key += 'x';
        customThemes[key] = { name, emoji, solid:d.solid, subj };
        saveCustomThemes(); applyThemeCustomizations(); closeThemeEditor();
        setTheme(key); renderThemeManager();
        showToast(`✓ '${name}' 테마를 만들었어요`);
        return;
    }
    if(d.mode==='custom'){
        customThemes[d.key] = { name, emoji, solid:d.solid, subj };
        saveCustomThemes();
    } else {
        // 기본 테마: 원본과 달라진 것만 조정값으로 저장(안 바꾼 과목은 계속 원본/대표색 기준 유지)
        const ov = {};
        if(d.solid.toUpperCase() !== THEMES_ORIG[d.key].solid.toUpperCase()) ov.solid = d.solid;
        const sj = {};
        THEME_EDIT_SUBJECTS.forEach(s=>{ if(subj[s].toUpperCase() !== normalizeSubjectBg(originalSubjectBg(d.key, s)).toUpperCase()) sj[s] = subj[s]; });
        if(Object.keys(sj).length) ov.subj = sj;
        if(Object.keys(ov).length){ themeOverrides[d.key] = ov; } else { delete themeOverrides[d.key]; msg = '바뀐 색이 없어요'; }
        saveThemeOverrides();
    }
    applyThemeCustomizations();
    const wasCurrent = d.key===currentTheme;
    closeThemeEditor();
    if(wasCurrent) setTheme(currentTheme); else renderThemePalette();
    renderThemeManager();
    showToast(msg);
}
function resetThemeOverrides(key){
    if(!themeOverrides[key]) return;
    delete themeOverrides[key];
    saveThemeOverrides(); applyThemeCustomizations(); closeThemeEditor();
    if(key===currentTheme) setTheme(currentTheme); else renderThemePalette();
    renderThemeManager();
    showToast('원래 색으로 되돌렸어요');
}
function deleteCustomTheme(key){
    const t = THEMES[key];
    if(!t || !t.custom) return;
    if(key===currentTheme){ showToast('사용 중인 테마는 삭제할 수 없어요'); return; }
    if(!window.confirm(`'${t.name}' 테마를 완전히 삭제할까요?\n(내가 만든 테마는 되돌릴 수 없어요)`)) return;
    delete customThemes[key];
    hiddenThemes = hiddenThemes.filter(k=>k!==key);
    saveCustomThemes(); saveHiddenThemes(); applyThemeCustomizations(); closeThemeEditor();
    renderThemePalette(); renderThemeManager();
    showToast('삭제했어요');
}

/* ── 테마 관리 창(색 조정 진입 / 새 테마 / 삭제·복원) ── */
function buildThemeManagerHtml(){
    const keys = getSortedThemeKeys();
    const shown  = keys.filter(k=>!hiddenThemes.includes(k) || k===currentTheme);
    const hidden = keys.filter(k=>hiddenThemes.includes(k) && k!==currentTheme);
    const chip = (k, right)=>{
        const t = THEMES[k];
        const tag = t.custom ? '내 테마' : (themeOverrides[k] ? '색 조정됨' : '');
        return `<div class="flex items-center gap-1.5 rounded-xl px-2.5 py-2" style="background:#f8fafc;border:1px solid #eef2f6;">
            <button onclick="openThemeEditor('${k}')" aria-label="${escThemeText(t.name)} 색 조정" class="flex items-center gap-2 flex-1 min-w-0 text-left">
                <span style="width:20px;height:20px;border-radius:9999px;background:${t.solid};flex-shrink:0;box-shadow:inset 0 0 0 1px rgba(15,23,42,.08);"></span>
                <span class="flex-1 min-w-0">
                    <span class="block text-[12px] font-black text-slate-600 truncate">${escThemeText(t.name)}</span>
                    ${tag ? `<span class="block text-[9px] font-black" style="color:var(--pc-solid)">${tag}</span>` : ''}
                </span>
            </button>
            ${right}
        </div>`;
    };
    const delBtn = (k)=> THEMES[k].custom
        ? `<button onclick="deleteCustomTheme('${k}')" aria-label="${escThemeText(THEMES[k].name)} 완전 삭제" class="shrink-0 text-[18px] leading-none font-black text-slate-300 px-1.5 py-0.5">×</button>`
        : `<button onclick="hideTheme('${k}')" aria-label="${escThemeText(THEMES[k].name)} 삭제" class="shrink-0 text-[18px] leading-none font-black text-slate-300 px-1.5 py-0.5">×</button>`;
    const shownHtml = shown.map(k=> chip(k, k===currentTheme
        ? `<span class="text-[10px] font-black shrink-0" style="color:var(--pc-solid)">사용 중</span>`
        : delBtn(k)
    )).join('');
    const hiddenHtml = hidden.length ? `
        <div class="flex items-center justify-between mt-5 mb-2">
            <div class="text-[12px] font-black text-slate-500">삭제한 테마 ${hidden.length}개</div>
            <button onclick="restoreAllThemes()" class="text-[11px] font-black px-2.5 py-1 rounded-full" style="background:rgba(var(--pc-solid-rgb),.14);color:var(--pc-solid)">모두 복원</button>
        </div>
        <div class="grid grid-cols-2 gap-2">${hidden.map(k=>chip(k,`<button onclick="showTheme('${k}')" class="shrink-0 text-[11px] font-black px-1.5 py-0.5" style="color:var(--pc-solid)">복원</button>`)).join('')}</div>` : '';
    return `<div style="width:100%;max-width:520px;max-height:82vh;overflow:auto;background:#fff;border-radius:22px 22px 0 0;padding:18px 16px calc(26px + env(safe-area-inset-bottom, 0px));box-shadow:0 -10px 40px rgba(15,23,42,.18);">
        <div class="flex items-center justify-between mb-1">
            <div class="text-[16px] font-black text-slate-800">테마 관리</div>
            <button onclick="closeThemeManager()" aria-label="닫기" class="text-[22px] leading-none font-black text-slate-400 px-2">×</button>
        </div>
        <div class="text-[11px] font-bold text-slate-400 mb-3">테마를 누르면 색을 조정할 수 있어요. ×는 기본 테마는 숨김(복원 가능), 내 테마는 완전 삭제예요.</div>
        <button onclick="openThemeCreator()" class="w-full text-[13px] font-black py-2.5 rounded-xl mb-4" style="background:rgba(var(--pc-solid-rgb),.14);color:var(--pc-solid)">＋ 새 테마 만들기</button>
        <div class="text-[12px] font-black text-slate-500 mb-2">사용할 테마 ${shown.length}개</div>
        <div class="grid grid-cols-2 gap-2">${shownHtml}</div>
        ${hiddenHtml}
    </div>`;
}
function renderThemeManager(){
    const ov = document.getElementById('theme-manager');
    if(ov) ov.innerHTML = buildThemeManagerHtml();
}
function openThemeManager(){
    let ov = document.getElementById('theme-manager');
    if(!ov){
        ov = document.createElement('div');
        ov.id = 'theme-manager';
        ov.style.cssText = 'position:fixed;inset:0;z-index:200;background:rgba(15,23,42,.45);display:none;align-items:flex-end;justify-content:center;';
        ov.onclick = (e)=>{ if(e.target===ov) closeThemeManager(); };
        document.body.appendChild(ov);
    }
    ov.innerHTML = buildThemeManagerHtml();
    ov.style.display = 'flex';
}
...
function hideTheme(key){
    if(THEMES[key]?.custom){ deleteCustomTheme(key); return; }
    if(key===currentTheme){ showToast('사용 중인 테마는 삭제할 수 없어요'); return; }
    if(!hiddenThemes.includes(key)) hiddenThemes.push(key);
    saveHiddenThemes(); renderThemePalette(); renderThemeManager();
}
...
function getThemeAccent(){
    ...
    return THEMES[currentTheme]?.emoji || accents[currentTheme] || '✦';
}
...
// ★ 사용자가 특정 날짜에서 자동배치된 진도 태스크를 직접 삭제했을 때 "이 항목-이 날짜는 다시 자동으로 채우지 마라"고 기억해두는 목록
let excludedAutoDates = JSON.parse(localStorage.getItem('young_excluded_auto_dates')) || {}; // { [itemId]: [dateStr, ...] }
...
let recordsYearGridYear = new Date().getFullYear(); // ★ 연간 월별 총계 그리드용 연도

function renderRecordsTrend(container){
    const N=30;
    const days=[];
    for(let i=N-1;i>=0;i--){
    ...
    const secs=days.map(ds=>getTotalTrackerSecs(ds));
    ...
    const pts=hours.map((h,i)=>{
        ...
        const y=H-PAD - (h/maxH)*(H-PAD*2);
        return `${x.toFixed(1)},${y.toFixed(1)}`;
    }).join(' ');
    const areaPts=`${PAD},${H-PAD} ${pts} ${W-PAD},${H-PAD}`;

    container.innerHTML=`<div class="card p-5 mb-6">
        <div class="flex items-center justify-between mb-1">
            <h3 class="text-[17px] font-black text-pc">최근 30일 추이</h3>
            <div class="text-[13px] font-bold text-slate-400">하루 평균 <span class="text-pc font-black">${fmtHHMM(Math.round(avgH*3600))}</span></div>
        </div>
        <svg viewBox="0 0 ${W} ${H}" style="width:100%;height:160px;" preserveAspectRatio="none">
            <polygon points="${areaPts}" fill="${pcAlpha(0.12)}" />
            <polyline points="${pts}" fill="none" stroke="var(--pc-solid)" stroke-width="1.4" vector-effect="non-scaling-stroke" />
        </svg>
        <div class="flex justify-between text-[11px] font-bold text-slate-300 mt-1">
            <span>${days[0].slice(5)}</span>
            <span>${days[Math.floor(N/2)].slice(5)}</span>
            <span>${days[N-1].slice(5)}</span>
        </div>
    </div>`;
}

// ★ 기간 선택 비교 — 두 기간을 골라서 총 시간/하루 평균을 나란히 비교
let periodCompareA = { start: null, end: null };
let periodCompareB = { start: null, end: null };
function updatePeriodCompare(which, field, val){
    (which==='A'?periodCompareA:periodCompareB)[field]=val;
    renderRecords();
}
function computePeriodStats(range){
    ...
    while(cur<=e){
        total += getTotalTrackerSecs(toKey(cur));
        days++;
        cur.setDate(cur.getDate()+1);
    }
    return { total, days, avg: days>0 ? total/days : 0 };
}
function renderRecordsPeriodCompare(container){
    if(!periodCompareA.start){
        const d=new Date(); d.setDate(d.getDate()-29);
        periodCompareA = { start: toKey(d), end: TODAY };
    }
    const statsA = computePeriodStats(periodCompareA);
    const statsB = computePeriodStats(periodCompareB);

    const card = (label, color, range, which, stats) => `
        <div class="card p-4">
            <div class="flex items-center gap-2 mb-2">
                <span style="width:12px;height:12px;border-radius:4px;background:${color};"></span>
                <span class="text-[15px] font-black text-slate-600">${label}</span>
            </div>
            <div class="flex items-center gap-2 mb-3">
                <input type="date" value="${range.start||''}" onchange="updatePeriodCompare('${which}','start',this.value)" class="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-[12px] font-bold outline-none focus:border-pc">
                <span class="text-slate-300 text-xs">~</span>
                <input type="date" value="${range.end||''}" onchange="updatePeriodCompare('${which}','end',this.value)" class="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-[12px] font-bold outline-none focus:border-pc">
            </div>
            ${stats ? `
            <div class="flex justify-around text-center">
                <div>
                    <div class="text-[11px] font-bold text-slate-400">총 시간</div>
                    <div class="text-[16px] font-black" style="color:${color}">${fmtHHMMSS(stats.total)}</div>
                </div>
                <div>
                    <div class="text-[11px] font-bold text-slate-400">하루 평균</div>
                    <div class="text-[16px] font-black" style="color:${color}">${fmtHHMMSS(Math.round(stats.avg))}</div>
                </div>
            </div>` : `<p class="text-center text-slate-300 text-xs py-3">기간을 선택해주세요</p>`}
        </div>`;

    container.innerHTML=`<div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ${card('기간 A', '#3B9EFF', periodCompareA, 'A', statsA)}
        ${card('기간 B', '#22B368', periodCompareB, 'B', statsB)}
    </div>`;
}

function renderRecordsYearGrid(container){
    const monthNames=['1월','2월','3월','4월','5월','6월','7월','8월','9월','10월','11월','12월'];
    const nowY=new Date().getFullYear(), nowM=new Date().getMonth();
    let maxSec=1;
    const totals=monthNames.map((_,m)=>{
        const sec=getMonthTotalSecs(new Date(recordsYearGridYear, m, 1));
        if(sec>maxSec) maxSec=sec;
        return sec;
    });
    const cells=monthNames.map((label,m)=>{
        const sec=totals[m];
        const isCurrent = recordsYearGridYear===nowY && m===nowM;
        const alpha = sec>0 ? 0.15 + (sec/maxSec)*0.55 : 0;
        return `<div onclick="recordsYear=${recordsYearGridYear};recordsMonth=${m};renderRecords();" class="rounded-2xl p-3 text-center cursor-pointer transition-all hover:opacity-80" style="background:${sec>0?pcAlpha(alpha):'#f8fafc'};${isCurrent?'outline:2px solid var(--pc);outline-offset:-2px;':''}">
            <div class="text-[13px] font-black text-slate-500 mb-1">${label}</div>
            <div class="text-[13px] font-black" style="color:${sec>0?'var(--pc-solid)':'#cbd5e1'}">${sec>0?fmtHHMMSS(sec):'00:00:00'}</div>
        </div>`;
    }).join('');

    container.innerHTML=`<div class="card p-5 mb-6">
        <div class="flex items-center justify-center gap-4 mb-4">
            <button onclick="recordsYearGridYear--;renderRecords();" class="text-slate-300 hover:text-pc text-lg font-black px-2">◀</button>
            <span class="text-[19px] font-black text-slate-700">${recordsYearGridYear}년</span>
            <button onclick="recordsYearGridYear++;renderRecords();" class="text-slate-300 hover:text-pc text-lg font-black px-2">▶</button>
        </div>
...
// ★ 예전 버그로 인해 시작일(startDate)과 회차 시작일(passStartDate)이 어긋난 채 저장된
//   기존 항목들을 자동으로 바로잡는다 (1회독인데 두 값이 다르면 시작일 기준으로 맞춤)
(function fixPassStartDrift(){
    if(!progressItems || !progressItems.length) return;
    let changed=false;
    progressItems=progressItems.map(p=>{
        if((p.currentPass||1)<=1 && p.startDate && p.passStartDate && p.passStartDate!==p.startDate){
            changed=true;
            return {...p, passStartDate:p.startDate};
        }
        return p;
    });
    if(changed) localStorage.setItem('young_progress_items', JSON.stringify(progressItems));
})();
...
// ★ 접기/펼치기(▶) 상태를 새로고침해도 유지하기 위한 공용 저장/로드 헬퍼
function loadBoolPref(key, def){ const v=localStorage.getItem('pref_'+key); return v===null?def:v==='1'; }
function saveBoolPref(key, val){ try{ localStorage.setItem('pref_'+key, val?'1':'0'); }catch{} }
// ★ 진도 탭과 완전히 동일한 스타일의 접기/펼치기 화살표(▶) — isOpen이면 90도 회전(▼처럼 아래를 향함)
function collapseArrowHtml(onclickFn, isOpen, extraStyle){
    return `<button onclick="${onclickFn}" style="background:transparent;border:none;color:#cbd5e1;font-size:12px;font-weight:900;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;transform:rotate(${isOpen?90:0}deg);transition:transform .15s;${extraStyle||''}">▶</button>`;
}
let dayTasksCollapsed = !loadBoolPref('dayTasksOpen', true); // 일간 플래너 — 과목별 세부 항목 전체 접기/펼치기 상태
function toggleDayTasksCollapse(){
    dayTasksCollapsed = !dayTasksCollapsed;
    saveBoolPref('dayTasksOpen', !dayTasksCollapsed);
    renderPlanner();
}
let taskControlsCollapsed = !loadBoolPref('taskControlsOpen', true); // 일간 플래너 — 목표~실제 입력칸(전체 과목) 접기/펼치기 상태
function toggleTaskControlsCollapse(){
    taskControlsCollapsed = !taskControlsCollapsed;
    saveBoolPref('taskControlsOpen', !taskControlsCollapsed);
    renderPlanner();
}
let conditionCardCollapsed = !loadBoolPref('conditionCardOpen', true); // 일간 플래너 — 오늘 컨디션 카드 접기/펼치기 상태
function toggleConditionCardCollapse(){
    conditionCardCollapsed = !conditionCardCollapsed;
    saveBoolPref('conditionCardOpen', !conditionCardCollapsed);
    renderPlanner();
}
function toggleReviewCounterCollapse(){
    const grid=document.getElementById('review-counter');
    const btn=document.getElementById('review-counter-toggle-btn');
    if(!grid) return;
    const collapsed=grid.style.display==='none';
    grid.style.display=collapsed?'':'none';
    if(btn) btn.style.transform=collapsed?'rotate(0deg)':'rotate(-90deg)';
}
// ★ "일간 달성도(진도 요약)" 카드를 상황에 맞는 위치로 옮긴다.
//   일간 뷰: 상중하 버튼과 날짜/시간 카드 사이. 주간/월간 뷰: 원래 기본 위치(플래너 영역 바로 아래).
function positionProgressSummary(anchorId){
    const sec=document.getElementById('progress-summary-section');
    const anchor=document.getElementById(anchorId);
    if(sec && anchor) anchor.appendChild(sec);
}
...
function getTaskCompletion(dateKey, idx, task){
    ...
    if(pages){
        ...
        if(actual!==undefined && actual!==''){
            const span=pages.end - pages.start + 1; // ★ 시작~끝을 포함(inclusive)해서 셈 (예: 1~3강 = 3강)
            if(span<=0) return (+actual>=pages.end) ? 1 : 0; // 범위가 잘못된 경우엔 도달 여부로만 판단
            const ratio=(+actual - pages.start + 1)/span;
...
// ★ 같은 과목+같은 항목명으로 여러 개(회독 시리즈 등) 등록돼 있으면 find()는 첫 번째만 돌려줘서
//   실제 자동배치를 담당하는 항목과 다른 항목이 잡힐 수 있다. 태스크가 '[과목] 항목명 ' 으로 시작하면
//   그 접두어와 정확히 일치하는 항목을 전부 돌려준다(못 찾으면 예전처럼 이름 포함 여부로 폴백).
function findAllMatchingProgressItems(subject, fullTaskText){
    if(!subject || !fullTaskText) return [];
    const canonSub = canonicalSubjectName(subject);
    const body = fullTaskText.replace(/^\[.*?\]\s*/,'');
    const strict = progressItems.filter(p => p.subject===canonSub && fullTaskText.startsWith(`[${p.subject}] ${p.name} `));
    if(strict.length) return strict;
    return progressItems.filter(p => p.subject===canonSub && body.includes(p.name));
}
// 삭제 기록은 항목 id뿐 아니라 '[과목] 항목명' 접두어로도 남겨서, 같은 이름의 다른 회독 항목이 다시 채우지 못하게 한다
function autoExclKey(item){ return `P:[${item.subject}] ${item.name}`; }
function isAutoExcluded(item, dateStr){
    return !!(excludedAutoDates[item.id]?.includes(dateStr) || excludedAutoDates[autoExclKey(item)]?.includes(dateStr));
}
...
function syncPlannerGoalsForCondition(date){
    ...
    day.tasks=day.tasks.map((t,idx)=>{
        ...
        let newEnd=Math.min(item.total, pages.start + prog.todayTarget - 1);
        const inGap=(prog.gaps||[]).find(g=>g[0]<=pages.start && pages.start<=g[1]);
        if(inGap) newEnd=Math.min(newEnd, inGap[1]); // 건너뛴 구간을 채우는 목표는 그 구간 끝을 넘지 않게
    ...
    if(editingProgressItemId){
        progressItems=progressItems.map(p=> p.id===editingProgressItemId
            ? {...p, subject:canonicalSubjectName(subject), name, total, unit, startDate, deadline, daysPerWeek, studyDays,
               ...
               // ★ 아직 1회독(첫 회차)이면 passStartDate는 항상 새로 입력한 시작일을 따라간다.
               //   (2회독 이상으로 넘어간 뒤라면 그 회차가 시작된 날짜를 그대로 유지해야 하므로 건드리지 않음)
               passStartDate: (p.currentPass||1) > 1 ? (p.passStartDate || startDate || TODAY) : (startDate || TODAY)}
            : p);
        clearAutoScheduleFor(editingProgressItemId, `[${canonicalSubjectName(subject)}] ${name}`, true); // 요일/주간일수/시작일/마감일이 바뀌었을 수 있으니, 이미 깔아둔 미래 태스크까지 지우고 자동 배치를 다시 계산
        showToast('✓ 수정 완료');
    } else {
...
// ★ 건너뛴 구간을 지정해서 플래너(선택한 날)에 넣기
function addGapGoalToPlanner(id, gs, ge){
    const item=progressItems.find(p=>p.id===id);
    if(!item) return;
    const targetDate = (currentTab==='planner' && currentView==='day' && selectedDate) ? selectedDate : TODAY;
    const taskText = `[${item.subject}] ${item.name} ${gs}~${ge}${item.unit}`;
    ...
    if(day.tasks.includes(taskText)){
        ...
        saveAll();
        showToast(`✓ ${targetDate} 플래너에 건너뛴 구간 추가됨`);
    }
    goToDay(targetDate);
}
// ★ 건너뛴 구간을 '이미 했거나 안 하기로 한 구간'으로 처리 — 진도율에 포함되고 자동 추천에서도 빠진다
function ackProgressGap(id, gs, ge){
    const item=progressItems.find(p=>p.id===id);
    if(!item) return;
    if(!window.confirm(`${gs}~${ge}${item.unit}을(를) 이미 했거나 안 하기로 한 구간으로 처리할까요?\n처리하면 진도율에 포함되고 자동 추천에서도 빠져요.`)) return;
    progressItems=progressItems.map(p=>p.id===id ? {...p, ackGaps:[...(p.ackGaps||[]), [gs,ge]]} : p);
    saveProgressItems();
    syncFutureScheduleWithActualProgress();
    showToast('✓ 완료 처리했어요');
}
function resetAckGaps(id){
    progressItems=progressItems.map(p=>p.id===id ? {...p, ackGaps:[]} : p);
    saveProgressItems();
    syncFutureScheduleWithActualProgress();
    showToast('다시 미완료로 되돌렸어요');
}
...
function addItemGoalToPlanner(id){
    ...
    let start, end;
    if(prog.current<item.total){ start=prog.current+1; end=Math.min(item.total, prog.current+amount); }
    else if(prog.gaps.length){ start=prog.gaps[0][0]; end=Math.min(prog.gaps[0][1], start+amount-1); } // 새 진도를 다 끝냈으면 건너뛴 구간부터
    else { showToast('이미 완료된 항목이에요'); return; }
...
function clearAutoScheduleFor(itemId, itemPrefix, keepUserTasks){
    ...
    if(rec && itemPrefix){
        ...
        (rec.dates||[]).forEach(dateStr=>{
            ...
            if(!day) return;
            const g=rec.gen?.[dateStr];
            for(let i=day.tasks.length-1;i>=0;i--){
                const t=day.tasks[i];
                if(!t.startsWith(prefix)) continue;
                // ★ 이미 완료 체크/실제값을 입력한 태스크는 실제 학습 기록이므로 지우지 않는다
                const k=`${dateStr}-${i}`;
                if(completedTasks[k] || (pageActuals[k]!==undefined && pageActuals[k]!=='')) continue;
                // ★ 항목 수정 시에는 사용자가 직접 넣거나 고친 태스크(자동배치가 만든 원문과 다른 것)는 남겨둔다
                if(keepUserTasks && g!==undefined && t!==g) continue;
                day.tasks.splice(i,1);
                reindexDayRecordsAfterRemoval(dateStr, i);
                dataChanged=true;
            }
        });
        if(dataChanged){
...
function autoScheduleWeek(){
    ...
    progressItems.forEach(item=>{
        ...
        // (이미 끝난 항목이면 아래에서 새로 배치하지는 않고, 남아 있는 낡은 자동 제안만 정리한다)
        ...
        // ★ 계획 상태: fwd = 지금까지 앞으로 나아간 지점(=오늘까지 실제 기록한 최종 도달치),
        //   gapQueue = 중간에 건너뛰어 아직 안 한 구간들.
        //   자동 추천은 '앞으로 나아가는 진도'를 먼저 이어가고, 그걸 다 끝내면 건너뛴 구간을 차례로 다시 잡아준다.
        let fwd=prog.current;
        let gapQueue=(prog.gaps||[]).map(g=>[g[0],g[1]]);
        const hasWork=()=> fwd<item.total || gapQueue.length>0;
        const nextStart=()=> fwd<item.total ? fwd+1 : (gapQueue.length ? gapQueue[0][0] : null);
        const takeNext=(size)=>{
            if(fwd<item.total) return {s:fwd+1, e:Math.min(item.total,fwd+size), gap:false};
            if(gapQueue.length){ const g=gapQueue[0]; return {s:g[0], e:Math.min(g[1], g[0]+size-1), gap:true}; }
            return null;
        };
        const consumeAuto=(r)=>{
            if(!r.gap){ fwd=Math.max(fwd,r.e); return; }
            gapQueue[0][0]=r.e+1;
            if(gapQueue[0][0]>gapQueue[0][1]) gapQueue.shift();
        };
        // 사용자가 직접 잡은 범위: 지금 진도보다 앞으로 뛰어넘어 시작하면 사이 구간을 '건너뜀'으로 남기고,
        // 건너뛴 구간 안쪽을 직접 잡으면 그 부분은 추천에서 뺀다
        const consumeUser=(ps,pe)=>{
            if(ps>fwd+1 && fwd<item.total) gapQueue.push([fwd+1, Math.min(ps-1,item.total)]);
            fwd=Math.max(fwd, Math.min(pe,item.total));
            gapQueue=mergeIntervals(subtractIntervals(gapQueue,[[ps,pe]]));
        };

        // ★ 자동배치가 관리하는 건 '이 항목의 진도 범위 태스크'(예: "[행정법] 요플 159~210p")뿐이다.
        //   "[행정법] 요플 복습"처럼 범위가 없는 태스크는 자동배치와 무관하므로 건드리지 않는다.
        const isPlanTask=(t)=>{
            if(!t.startsWith(prefix+' ')) return false;
            const p=parseTaskPages(t);
            return !!(p && p.unit===item.unit);
        };
        // ★ rec.gen[날짜] = 자동배치가 그 날짜에 마지막으로 만든 태스크 원문.
        //   원문과 똑같으면 '손대지 않은 자동 제안', 다르면(직접 추가/수정) '사용자 소유'로 보고 절대 지우지 않는다.
        //   (null = 사용자가 그 날짜의 자동 제안을 직접 삭제함)
        if(!rec.gen) rec.gen={};
        ...
        horizonDates.forEach(dateStr=>{
            ...
            const planIdxs=[];
            (existingDay?.tasks||[]).forEach((t,i)=>{ if(isPlanTask(t)) planIdxs.push(i); });

            // 이 기능이 생기기 전에 자동으로 깔린 태스크(gen 기록 없음)는 그대로 '자동 제안'으로 간주해 이어받는다
            if(planIdxs.length && rec.gen[dateStr]===undefined && rec.dates.includes(dateStr)){
                rec.gen[dateStr]=existingDay.tasks[planIdxs[0]];
            }
            const genText=rec.gen[dateStr];

            if(planIdxs.length){
                const recordedIdxs=[], userIdxs=[], autoIdxs=[];
                planIdxs.forEach(i=>{
                    const k=`${dateStr}-${i}`;
                    const recorded=!!completedTasks[k] || (pageActuals[k]!==undefined && pageActuals[k]!=='');
                    if(recorded) recordedIdxs.push(i);
                    else if(existingDay.tasks[i]!==genText) userIdxs.push(i);
                    else autoIdxs.push(i);
                });

                if(recordedIdxs.length){
                    // ★ 이미 실제로 기록(완료 체크/실제값 입력)된 날짜는 절대 건드리지 않는다.
                    //   cursor는 이미 prog.current(전체 실제 도달치)로 시작했으므로 그대로 둔다.
                    if(!rec.dates.includes(dateStr)) rec.dates.push(dateStr);
                    return;
                }
                if(userIdxs.length){
                    // ★ 사용자가 직접 추가/수정한 범위(예: 202~250p)는 그대로 존중한다.
                    //   - 지금 진도보다 앞으로 뛰어넘어 시작했다면(158까지 하고 202부터 등) 그 사이(159~201)는
                    //     '건너뛴 구간'으로 남기고, 뒤 날짜의 자동 계획은 이 범위의 끝(251~)에서 이어진다.
                    //   - 건너뛴 구간을 채우는 범위를 직접 넣었다면 그 구간은 추천에서 빼준다.
                    let advances=false;
                    userIdxs.forEach(i=>{
                        const up=parseTaskPages(existingDay.tasks[i]);
                        if(!up) return;
                        if(up.end>fwd) advances=true;
                        consumeUser(up.start, up.end);
                    });
                    if(advances && autoIdxs.length){
                        // 같은 날 손대지 않은 자동 제안이 남아 있으면(직접 넣은 것과 겹침) 중복이므로 정리
                        autoIdxs.slice().sort((a,b)=>b-a).forEach(i=>{
                            existingDay.tasks.splice(i,1);
                            reindexDayRecordsAfterRemoval(dateStr, i);
                        });
                        rec.gen[dateStr]=null;
                        changed=true;
                        return;
                    }
                    if(!autoIdxs.length) return;
                    // 건너뛴 구간만 채워 넣은 날이면, 남아 있는 자동 제안은 아래에서 그대로 이어서 검사한다
                }
                // 손대지 않은 자동 제안만 있는 경우: 지금 이어질 지점과 맞으면 유지, 아니면 낡은 계획이므로 지우고 새로 짠다
                const p=parseTaskPages(existingDay.tasks[autoIdxs[0]]);
                if(autoIdxs.length===1 && p && p.start===nextStart()){
                    consumeAuto({s:p.start, e:p.end, gap:fwd>=item.total});
                    return;
                }
                autoIdxs.slice().sort((a,b)=>b-a).forEach(i=>{
                    existingDay.tasks.splice(i,1);
                ...
                rec.dates=rec.dates.filter(d=>d!==dateStr);
                delete rec.gen[dateStr];
                changed=true;
                // 아래로 통과시켜서 이 날짜를 지금 이어질 지점 기준으로 새로 배치한다
            }
            if(!hasWork()) return; // 더 배치할 진도(앞으로 나아갈 것/건너뛴 구간)가 없음
            if(dateStr<TODAY) return; // 지나간 날짜는 새로 채워넣지 않음(이미 지난 일정)
            if(isAutoExcluded(item, dateStr)) return; // ★ 사용자가 이 날짜에서 직접 삭제했던 경우 — 다시 채우지 않음
            ...
            const r=takeNext(Math.max(1,baseTarget));
            if(!r || r.e<r.s) return;

            let day=scheduleData.find(s=>s.date===dateStr);
            if(!day){ day={date:dateStr, tasks:[]}; scheduleData.push(day); }
            const text=`${prefix} ${r.s}~${r.e}${item.unit}`;
            if(!day.tasks.includes(text)){
                day.tasks.push(text);
                rec.gen[dateStr]=text;
                changed=true;
            }
            if(!rec.dates.includes(dateStr)) rec.dates.push(dateStr);
            consumeAuto(r);
        ...
        Object.keys(rec.gen).forEach(d=>{ if(d<addDays(TODAY,-60)) delete rec.gen[d]; });
    });
...
// ★ 이 항목을 일간 플래너에 실제로 처음 기록한 날짜, 100% 채워서 끝낸 날짜, 그리고
//   기간 내 날짜별 실제 도달치를 계산한다. (기록 탭 "진도" 화면에서 사용)
function getItemActualRecord(item){
    const passStart = item.passStartDate || item.startDate || null;
    const isMultiPass = (item.totalPasses||1) > 1;
    const isSeriesItem = progressItems.filter(p=>p.subject===item.subject && p.name===item.name).length > 1;
    const needsDateFilter = isMultiPass || isSeriesItem;

    let firstDate=null, completeDate=null, current=0;
    const dailyReached={}; // { dateStr: 그날까지 누적 도달치 }
    const allRanges=[];    // 실제로 한 범위들
    const numDates={};     // { 번호: 그 번호를 처음 실제로 한 날짜 } — 건너뛴 번호는 여기 없음

    const sorted=[...scheduleData].sort((a,b)=> a.date<b.date?-1:(a.date>b.date?1:0));
    sorted.forEach(day=>{
        if(needsDateFilter){
            if(passStart && day.date < passStart) return;
            if(item.deadline && day.date > item.deadline) return;
        }
        (day.tasks||[]).forEach((t,idx)=>{
            ...
            } else if(isDone && txt.trim()===item.name){
                reached=item.total;
            }
            if(reached>0){
                if(!firstDate || day.date<firstDate) firstDate=day.date;
                if(reached>current) current=reached;
                if(!dailyReached[day.date] || current>dailyReached[day.date]) dailyReached[day.date]=current;

                let doneRange=null;
                if(pages && pages.unit===item.unit){ if(reached>=pages.start) doneRange=[pages.start, reached]; }
                else if(reached>=item.total && item.total>0) doneRange=[1,item.total];
                if(doneRange){
                    allRanges.push(doneRange);
                    const hi=Math.min(doneRange[1], item.total);
                    for(let n=Math.max(1,doneRange[0]); n<=hi; n++){ if(!numDates[n]) numDates[n]=day.date; }
                }
                // ★ 건너뛴 구간이 남아 있으면 끝까지 도달했어도 '완료'가 아니다
                if(!completeDate && current>=item.total && analyzeCoverage(allRanges,item).gapSum===0) completeDate=day.date;
            }
        });
    });
    const cov=analyzeCoverage(allRanges,item);
    return {firstDate, completeDate, current, dailyReached, numDates, gaps:cov.gaps, gapSum:cov.gapSum};
}

function renderProgressPassTable(container){
    if(!progressItems.length){
        container.innerHTML=`<div class="card p-10 text-center text-slate-300">등록된 진도 항목이 없어요</div>`;
        return;
    }
    // 과목+항목명이 같은 것끼리 "회독 시리즈"로 묶기
    const seriesMap={}; // key: subject|||name -> [items...]
    const order=[];
    progressItems.forEach(p=>{
        const key=`${p.subject}|||${p.name}`;
        if(!seriesMap[key]){ seriesMap[key]=[]; order.push(key); }
        seriesMap[key].push(p);
    });

    let maxPasses=1;
    const rows=order.map(key=>{
        const items=seriesMap[key].slice().sort((a,b)=> (a.startDate||'').localeCompare(b.startDate||''));
        maxPasses=Math.max(maxPasses, items.length);
        return {subject:items[0].subject, name:items[0].name, items};
    });

    let head=`<th class="text-left px-3 py-2">과목</th><th class="text-left px-3 py-2">항목명</th>`;
    for(let i=0;i<maxPasses;i++) head+=`<th class="text-center px-3 py-2">회독 ${i+1}</th>`;

    let body='';
    let lastSubject=null;
    rows.forEach(r=>{
        const subjColor=getSubjTextColor(r.subject)||'var(--pc)';
        body+=`<tr class="border-t border-slate-100">
            <td class="px-3 py-2 font-black" style="color:${subjColor}">${r.subject!==lastSubject?r.subject:''}</td>
            <td class="px-3 py-2 font-bold text-slate-600">${r.name}</td>`;
        lastSubject=r.subject;
        for(let i=0;i<maxPasses;i++){
            const item=r.items[i];
            if(!item){ body+=`<td class="px-3 py-2"></td>`; continue; }
            const rec=getItemActualRecord(item);
            if(!rec.firstDate){
                body+=`<td class="px-3 py-2 text-center text-slate-300 text-xs">대기</td>`;
            } else {
                const endLabel = rec.completeDate ? rec.completeDate.slice(5) : '진행중';
                const days = rec.completeDate ? (Math.round((new Date(rec.completeDate)-new Date(rec.firstDate))/86400000)+1) : null;
                body+=`<td class="px-3 py-2 text-center text-xs">
                    <div class="font-bold text-slate-700">${rec.firstDate.slice(5)} ~ ${endLabel}</div>
                    ${days!==null?`<div class="font-black" style="color:${subjColor}">${days}일</div>`:`<div class="text-slate-400">${rec.current}/${item.total}${item.unit}</div>`}
                </td>`;
            }
        }
        body+='</tr>';
    ...
    container.innerHTML=`<div class="card p-4 overflow-x-auto">
        <table class="w-full text-sm" style="border-collapse:collapse;">
            <thead><tr class="text-slate-400 text-xs">${head}</tr></thead>
            <tbody>${body}</tbody>
        </table>
    </div>`;
}

function renderProgressLectureView(container){
    if(!progressItems.length){
        container.innerHTML=`<div class="card p-10 text-center text-slate-300">등록된 진도 항목이 없어요</div>`;
        return;
    }
    if(!window._lectureViewItemId) window._lectureViewItemId = progressItems[0].id;
    const options = progressItems.map(p=>`<option value="${p.id}" ${p.id===window._lectureViewItemId?'selected':''}>[${p.subject}] ${p.name} (${p.startDate||'?'}~${p.deadline||'?'})</option>`).join('');

    const item = progressItems.find(p=>p.id===window._lectureViewItemId) || progressItems[0];
    const rec = getItemActualRecord(item);

    // ★ 강의번호(1~총분량) 순서대로, 그 번호에 실제로 도달한 날짜를 매핑
    // ★ 실제로 한 번호만 날짜를 표시한다. 건너뛴(미완료) 구간은 날짜 없이 주황색으로 표시한다.
    //   (맨 앞 구간은 기록이 시작되기 전에 이미 한 것으로 보고 첫 기록일로 채움)
    const countDate=new Array(item.total+1).fill(null);
    let firstCovered=null;
    for(let n=1;n<=item.total;n++){ if(rec.numDates[n]){ countDate[n]=rec.numDates[n]; if(firstCovered===null) firstCovered=n; } }
    if(firstCovered!==null && rec.firstDate){ for(let n=1;n<firstCovered;n++) countDate[n]=rec.firstDate; }
    const gapSet=new Set(), ackSet=new Set();
    (rec.gaps||[]).forEach(([gs,ge])=>{ for(let n=gs;n<=ge;n++) gapSet.add(n); });
    (item.ackGaps||[]).forEach(([gs,ge])=>{ for(let n=gs;n<=ge;n++) ackSet.add(n); });

    const COLS=20;
    let cellsHtml='';
    for(let n=1; n<=item.total; n++){
        const ds=countDate[n];
        let bg='#f8fafc', fg='#cbd5e1', label=n;
        if(ds){ bg=pcAlpha(0.14); fg='var(--pc-solid)'; label=ds.slice(5).replace('-','.'); }
        else if(gapSet.has(n)){ bg='#fff7ed'; fg='#c2410c'; }                       // 건너뛴(미완료) 구간
        else if(ackSet.has(n)){ bg=pcAlpha(0.06); fg='#94a3b8'; label='✓'; }         // 완료 처리한 구간
        cellsHtml+=`<div class="rounded-md text-center py-1.5" style="background:${bg};color:${fg};font-size:10px;font-weight:800;line-height:1.3;">
            <div>${label}</div>
        </div>`;
    }

    const periodLabel = item.startDate && item.deadline ? `${item.startDate.slice(5).replace('-','.')} ~ ${item.deadline.slice(5).replace('-','.')}` : '기간 미설정';
    const daysLabel = rec.firstDate && rec.completeDate ? Math.round((new Date(rec.completeDate)-new Date(rec.firstDate))/86400000)+1 : null;

    container.innerHTML=`<div class="card p-5">
        <select onchange="window._lectureViewItemId=this.value;renderRecords();" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm font-bold outline-none focus:border-pc mb-4">${options}</select>
        <div class="flex items-center justify-between mb-1">
            <div class="flex items-center gap-3">
                <span class="text-[15px] font-black text-slate-700">${item.subject}</span>
                <span class="text-[13px] font-bold text-slate-400">${item.name}</span>
            </div>
            <span class="text-[13px] font-bold text-slate-400">기간 <span class="font-black text-slate-600">${periodLabel}</span></span>
        </div>
        <div class="text-xs text-slate-400 mb-3 font-bold">
            ${rec.firstDate?`시작 ${rec.firstDate} · ${rec.completeDate?`완료 ${rec.completeDate} (${daysLabel}일 소요)`:`진행중 (${rec.current}/${item.total}${item.unit}${rec.gapSum>0?` · 건너뜀 ${rec.gapSum}${item.unit}`:''})`}`:'아직 기록 없음'}
        </div>
        <div style="display:grid;grid-template-columns:repeat(${COLS},1fr);gap:3px;">${cellsHtml || '<p class="text-slate-300 text-sm text-center py-6">총 분량이 설정되지 않았어요</p>'}</div>
    </div>`;
}

/* ═══════════════════════════════════════════════
   진도 '구간' 계산 유틸 — 실제로 한 범위들을 합쳐서 중간에 건너뛴(빈) 구간을 찾는다.
   예) 100~158p 기록 + 202~250p 기록 → 159~201p는 '건너뜀'(미완료)
   - 첫 기록 이전 구간은 기록 전에 이미 한 것으로 보고 건너뜀으로 치지 않는다.
   - item.ackGaps(사용자가 '완료 처리'한 구간)는 건너뜀에서 제외한다.
═══════════════════════════════════════════════ */
function mergeIntervals(list){
    const a=(list||[]).filter(r=>r && r[1]>=r[0]).map(r=>[r[0],r[1]]).sort((x,y)=>x[0]-y[0]);
    const out=[];
    a.forEach(r=>{
...
function subtractIntervals(base, cuts){
    let cur=(base||[]).map(r=>[r[0],r[1]]);
    (cuts||[]).forEach(([cs,ce])=>{
        const next=[];
        cur.forEach(([s,e])=>{
            if(ce<s || cs>e){ next.push([s,e]); return; }
...
function intervalSize(list){ return (list||[]).reduce((a,r)=>a+(r[1]-r[0]+1),0); }
function analyzeCoverage(ranges, item){
    const merged=mergeIntervals((ranges||[]).map(r=>[Math.max(1,r[0]), Math.min(item.total,r[1])]));
    const holes=[];
    for(let i=1;i<merged.length;i++) holes.push([merged[i-1][1]+1, merged[i][0]-1]);
    const gaps=subtractIntervals(holes, item.ackGaps||[]).filter(g=>g[1]>=g[0]);
    return {gaps, gapSum:intervalSize(gaps)};
}
...
function computeItemProgress(item, asOf){
    ...
    const ranges=[], rangesBefore=[]; // ★ 실제로 한 범위들(건너뛴 구간 계산용)
    const passStart = item.passStartDate || item.startDate || null;
    const isMultiPass = (item.totalPasses||1) > 1;
    // ★ 같은 과목+같은 항목명으로 여러 번(회독별로) 등록해둔 "시리즈" 항목인지 확인.
    //   시리즈면 totalPasses가 1이어도 서로 다른 회독이므로 기간으로 반드시 구분해야 한다.
    const isSeriesItem = typeof progressItems!=='undefined' && progressItems.filter(p=>p.subject===item.subject && p.name===item.name).length > 1;
    const needsDateFilter = isMultiPass || isSeriesItem;
    scheduleData.forEach(day=>{
        // ★ 다회독(2회독 이상) 또는 회독 시리즈로 등록된 항목만 자신의 시작~마감일로 걸러낸다.
        //   그 외(진짜 1회성 단일 항목)는 시작일 이전에 수동으로 기입한 기록도 그대로 진도에 반영한다.
        if(needsDateFilter){
            if(passStart && day.date < passStart) return;
            if(item.deadline && day.date > item.deadline) return;
        }
        (day.tasks||[]).forEach((t,idx)=>{
            ...
            if(pages && pages.unit===item.unit){
                ...
                reached=item.total; // ★ 항목명과 "완전히 똑같은" 태스크(페이지 표기 없이 그 자체)만 체크 완료 시 전체 완료로 간주.
                                      // "기심론 구문 복습"처럼 접미사가 붙은 태스크는 별개의 부가 태스크이므로 전체 완료로 치지 않음.
            }
            let doneRange=null;
            if(pages && pages.unit===item.unit){ if(reached>=pages.start) doneRange=[pages.start, reached]; }
            else if(reached>=item.total && item.total>0) doneRange=[1,item.total];
            if(doneRange){ ranges.push(doneRange); if(day.date<asOf) rangesBefore.push(doneRange); }
            if(reached>current){
                current=reached;
                lastDate=day.date;
            }
    ...
    // ★ 중간에 건너뛴(아직 안 한) 구간은 진도율·남은 분량에서 '미완료'로 계산한다
    const cov=analyzeCoverage(ranges, item);
    const covBefore=analyzeCoverage(rangesBefore, item);
    const doneUnits=Math.max(0, current-cov.gapSum);
    const doneUnitsBefore=Math.max(0, currentBeforeAsOf-covBefore.gapSum);
    let todayDone=Math.max(0, doneUnits-doneUnitsBefore);
    const remaining=Math.max(0,item.total-doneUnits);
    const pct=item.total>0 ? Math.max(0, Math.min(100, Math.round(doneUnits/item.total*100))) : 0;

    // 하루 평균 진도 (그동안 이 항목을 공부한 날 기준)
    const workedDays=workedDates.size;
    const avgPace = workedDays>0 ? doneUnits/workedDays : null;
    ...
    if(passStart){
        ...
        if(doneUnits>0) estimatedPassDays=Math.round(daysElapsedInPass/(doneUnits/item.total));
    }

    return {current, doneUnits, gaps:cov.gaps, gapSum:cov.gapSum, remaining, pct, lastDate, workedDays, avgPace, dailyTarget, dailyPace, dailyMinutes, daysUntilDeadline, studySessionsLeft, isRestDayToday, totalPasses, currentPass, estimatedPassDays, effectiveDaysPerWeek, todayDone, todayTarget, todayMinutes};
}
...
function renderProgress(){
    ...
    const formHtml=`<div class="card p-5 mb-5" id="pi-form-card">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-[1fr_1.2fr_0.7fr_0.7fr_auto] gap-2 mb-2">
            <input id="pi-subject-input" list="pi-subject-list" type="text" placeholder="과목 (예: 행정법)"
                class="bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-[13px] font-medium outline-none focus:border-pc col-span-2 lg:col-span-1">
            ...
            <input id="pi-total-input" type="number" placeholder="전체 분량"
                class="bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-[13px] font-medium outline-none focus:border-pc">
            <select id="pi-unit-input" onchange="updateMinutesLabel()" class="bg-slate-50 border border-slate-200 rounded-xl px-2 py-2 text-[13px] font-medium outline-none focus:border-pc">
    ...
    const sectionsHtml=subjectKeys.map(subName=>{
        ...
        const itemsHtml=list.map(c=>{
            ...
            if(dailyPace!==null && remaining>0){
                ...
                paceLineHtml = `<div class="text-[12px] font-bold mt-1 text-slate-400">🎯 전체 페이스: 하루 ${dailyPace}${item.unit}씩${dDayLabel}${avgLabel}${isBehind?' — 페이스업 필요!':''}</div>`;
            }

            // 오늘(혹은 조회 중인 날) 목표 — 컨디션·쉬는 날에 따라 달라짐
            let targetLineHtml='';
            if(remaining<=0){
                targetLineHtml = `<div class="text-[12px] font-bold mt-1" style="color:#16a34a">✓ 목표 분량 완료</div>`;
            } else if(isRestToday){
                targetLineHtml = '';
            ...
            // ★ 건너뛴(미완료) 구간 표시 + 플래너 추가 / 완료 처리 버튼
            let gapHtml='';
            if(c.gaps && c.gaps.length){
                gapHtml+=`<div class="mt-1.5 rounded-lg px-2.5 py-1.5" style="background:rgba(251,146,60,.12);">
                    <div class="text-[12px] font-black" style="color:#c2410c">⏭ 건너뛴 구간 ${c.gapSum}${item.unit} · 진도율에서 제외됨</div>
                    ${c.gaps.map(g=>`<div class="flex items-center gap-2 text-[12px] font-bold text-slate-500 mt-1">
                        <span class="flex-1 min-w-0">${g[0]}~${g[1]}${item.unit} <span class="text-slate-400">(${g[1]-g[0]+1}${item.unit})</span></span>
                        <button onclick="addGapGoalToPlanner('${item.id}',${g[0]},${g[1]})" class="text-[11px] font-black text-pc shrink-0">＋플래너</button>
                        <button onclick="ackProgressGap('${item.id}',${g[0]},${g[1]})" class="text-[11px] font-black text-slate-400 shrink-0">완료 처리</button>
                    </div>`).join('')}
                </div>`;
            }
            if(item.ackGaps && item.ackGaps.length){
                gapHtml+=`<div class="flex items-center gap-2 text-[11px] font-bold text-slate-400 mt-1">
                    <span class="flex-1 min-w-0">✓ 완료 처리한 구간 ${item.ackGaps.map(g=>`${g[0]}~${g[1]}`).join(', ')}${item.unit}</span>
                    <button onclick="resetAckGaps('${item.id}')" class="text-[11px] font-black text-slate-400 shrink-0">되돌리기</button>
                </div>`;
            }

            return `<div class="py-2 border-b border-slate-50 last:border-0">
                <div class="flex items-center justify-between gap-x-2 gap-y-1 mb-1 flex-wrap">
                    <div class="flex items-center gap-1.5 min-w-0 flex-1">
                        <span class="text-[15px] font-bold text-slate-700 truncate">${item.unit==='강'?'📺 ':''}${item.name}</span>
                        ${studyDaysLabel(item) ? `<span class="text-[12px] font-bold text-pc shrink-0">${studyDaysLabel(item)}</span>` : ''}
                        ${passBadge}
                    </div>
                    <span class="text-[12px] text-slate-400 font-bold shrink-0">${dateLabel}</span>
                    <span class="text-[12px] font-black px-2 py-0.5 rounded-full shrink-0" style="background:${st.bg};color:${st.color}">${st.label}</span>
                    <button onclick="addItemGoalToPlanner('${item.id}')" class="text-[12px] text-slate-400 hover:text-pc shrink-0">＋플래너</button>
                    <button onclick="startEditProgressItem('${item.id}')" class="text-[12px] text-slate-400 hover:text-pc shrink-0">수정</button>
                    <button onclick="toggleProgressItemHidden('${item.id}')" class="text-[12px] text-slate-400 hover:text-pc shrink-0">숨기기</button>
                    <button onclick="deleteProgressItem('${item.id}')" class="text-[12px] text-slate-400 hover:text-red-400 shrink-0">삭제</button>
                </div>
                <div class="flex items-center gap-2">
                    <div class="text-[13px] font-semibold text-slate-500 shrink-0" style="min-width:90px;">${c.doneUnits}/${item.total}${item.unit} · 남음 ${remaining}${item.unit}</div>
                    <div class="flex-1 prog-bg"><div class="prog-fill" style="width:${pct}%"></div></div>
                    <div class="text-[14px] font-black shrink-0" style="min-width:38px;text-align:right;color:${getSubjTextColor(subName)}">${pct}%</div>
                </div>
                ...
                ${passLineHtml}
                ${gapHtml}
                ${item.startDate && item.deadline ? `<div class="text-[12px] font-semibold text-slate-500 mt-1">${item.startDate.slice(5).replace('-','/')}~${item.deadline.slice(5).replace('-','/')} · 총 ${Math.round((new Date(item.deadline+'T00:00:00')-new Date(item.startDate+'T00:00:00'))/86400000)}일 · 주 ${effectiveDaysPerWeek}일${item.daysPerWeek?'':'(기본값)'}</div>` : ''}
            </div>`;
        }).join('');
        ...
        return `<div class="card p-5 mb-3" id="${cardId}">
            <div class="flex items-center justify-between mb-2 cursor-pointer select-none" onclick="toggleProgressSection('${subName}')">
                <div class="flex items-center gap-2 min-w-0">
                    <span class="px-3 py-1.5 rounded-full text-base font-black shrink-0" style="${badgeStyle}">${subName}</span>
                    <span class="text-[13px] text-slate-300 font-bold shrink-0">${list.length}개 항목</span>
                </div>
                <div class="flex items-center gap-2 shrink-0">
                    <span class="text-base font-black" style="color:${getSubjTextColor(subName)}">${subjPct}%</span>
                    <span class="text-slate-300 text-[13px] font-black" style="display:inline-block;transition:transform .15s;transform:rotate(${sectionCollapsed?0:90}deg)">▶</span>
                </div>
            </div>
            <div class="prog-bg mb-3"><div class="prog-fill" style="width:${subjPct}%"></div></div>
...
function cleanupStaleAutoPlacedTasks(){
    ...
    try{
        ...
        scheduleData.forEach(day=>{
            ...
            const keepFlags=day.tasks.map((t,idx)=>{
                const sub=getTaskSubject(t);
                const txt=t.replace(/^\[.*?\]\s*/,'');
                const item=findMatchingProgressItem(sub, txt);
                if(!item) return true; // 등록된 진도 항목과 매칭 안 되는 자유 입력 태스크는 그대로 둠
                // ★ 시스템이 이 날짜에 자동으로 깔아둔 기록이 있는 태스크만 정리 대상으로 삼는다.
                //   사용자가 +플래너 버튼이나 직접 입력으로 넣은 태스크는 절대 자동 삭제하지 않는다.
                const rec=store[item.id];
                const wasAutoPlaced = !!(rec && rec.dates && rec.dates.includes(day.date));
                if(!wasAutoPlaced) return true;
                // ★ 자동배치가 만든 원문과 다른 태스크(사용자가 직접 넣거나 고친 것)는 지우지 않는다.
                const genT=rec.gen?.[day.date];
                if(genT!==undefined && t!==genT) return true;
                // ★ 이미 완료 체크를 했거나 실제 진도값을 입력한 태스크는 실제 학습 기록이므로 절대 지우지 않는다.
                const actualKey=`${day.date}-${idx}`;
                const hasRecordedProgress = !!completedTasks[actualKey] || (pageActuals[actualKey]!==undefined && pageActuals[actualKey]!=='');
            ...
            if(keepFlags.every(Boolean)) return; // 지울 게 없으면 재인덱싱도 필요 없음

            // ★ 여러 개를 한꺼번에 지우는 경우, 지운 뒤의 새 인덱스로 실제값/완료기록을 옮겨준다
            const oldToNew={};
            let newIdx=0;
            keepFlags.forEach((keep,oldIdx)=>{ if(keep){ oldToNew[oldIdx]=newIdx; newIdx++; } });
            [completedTasks, pageActuals].forEach(rec=>{
                const moved={};
                Object.keys(rec).forEach(k=>{
                    if(!k.startsWith(day.date+'-')) return;
                    const ti=parseInt(k.split('-').pop());
                    if(oldToNew[ti]!==undefined){ moved[`${day.date}-${oldToNew[ti]}`]=rec[k]; }
                    delete rec[k];
                });
                Object.assign(rec, moved);
            });
            day.tasks=day.tasks.filter((_,idx)=>keepFlags[idx]);
            changed=true;
        });
        if(changed){
            localStorage.setItem('young_study_v4', JSON.stringify(scheduleData));
...
function calculateProgress(){
    ...
    if(currentView==='month'){
        ...
        filtered=scheduleData.filter(d=>d.date.startsWith(ym) && d.date<=TODAY); // ★ 아직 안 지난 미래 날짜는 평균에서 제외
        document.getElementById('progress-title').innerText='월간 달성도';
    }else if(currentView==='week'){
        const s=weekStart(weekRefDate),e=new Date(s);e.setDate(s.getDate()+7);
        filtered=scheduleData.filter(d=>{const o=new Date(d.date);return o>=s&&o<e&&d.date<=TODAY;}); // ★ 아직 안 지난 미래 날짜는 평균에서 제외
...
let reviewCounterCollapsed = !loadBoolPref('reviewCounterOpen', true);
function toggleReviewCounterCollapse(){
    reviewCounterCollapsed = !reviewCounterCollapsed;
    saveBoolPref('reviewCounterOpen', !reviewCounterCollapsed);
    applyReviewCounterCollapseState();
}
function applyReviewCounterCollapseState(){
    const grid=document.getElementById('review-counter');
    const btn=document.getElementById('review-counter-toggle-btn');
    if(grid) grid.style.display=reviewCounterCollapsed?'none':'';
    if(btn) btn.style.transform=`rotate(${reviewCounterCollapsed?0:90}deg)`;
}
...
/* ═══════════════════════════════════════════════
   TIMER — 백그라운드 복귀 시 실제 경과 시간만큼 따라잡기
   (iOS Safari 등은 화면이 꺼지거나 다른 탭/앱으로 가면 setInterval이
    멈추거나 느려져서 1초마다 카운트하는 방식만으로는 시간이 덜 쌓인다.
    화면이 다시 보이는 순간, 타이머를 켠 시점(_startSec)부터 지금까지의
    실제 경과 시간을 다시 계산해서 그만큼 한 번에 더해준다.)
═══════════════════════════════════════════════ */
function catchUpTimerOnResume(){
    if(!timerSubjects || !timerSubjects.length) return;
    const nowSec=getNowDaySec();
    let changed=false;
    timerSubjects=timerSubjects.map(s=>{
        if(!s.on || s._startSec===undefined) return s;
        const creditFrom = s._lastCreditedSec!==undefined ? s._lastCreditedSec : s._startSec;
        let elapsed=nowSec-creditFrom;
        if(elapsed<0) elapsed+=86400; // 자정을 넘긴 경우 보정
        if(elapsed<=0) return s;
        changed=true;
        // ★ _startSec(진짜 시작 시각)은 그대로 두고, 다음 정지 시 한 구간으로 기록되게 한다.
        //   (매번 로그를 새로 남기면 "따라잡기"가 실행될 때마다 흐름 차트/시간수정 목록이 잘게 쪼개짐)
        return {...s, spent:s.spent+elapsed, rem:Math.max(0,s.rem-elapsed), _lastCreditedSec:nowSec};
    });
    if(changed){
        renderTimer();
        updateLiveDayTimerDisplay();
...
document.addEventListener('visibilitychange', ()=>{ if(document.visibilityState==='visible') catchUpTimerOnResume(); });
window.addEventListener('focus', catchUpTimerOnResume);
window.addEventListener('pageshow', catchUpTimerOnResume);
...
function getSubjColor(name){
    ...
    if(bgTable[key]) return bgTable[key]; // 테마에 이 과목 색이 따로 지정돼 있으면 사용(컬러표 테마의 '기타', 사용자가 조정한 한국사·헌법·경제·각론 등)
...
// 배경색 위에 올릴 글씨색: 같은 색상(Hue)에서 채도를 올리고 명도를 낮춘다.
// 배경이 아주 밝으면(기존 테마 전부 해당) 명도 상한 .32 그대로, 배경이 더 어두우면 그만큼 글씨도 더 진하게 해서 대비를 유지한다.
function paletteTextOn(bgHex){
    const [h,s,bgL]=hexToHsl(bgHex);
    const hi=Math.min(.32, Math.max(.20, bgL-.45));
    // 회색·흰색에 가까운 배경(사용자가 고른 색)은 억지로 채도를 올리면 붉은 글씨가 되므로 중립 진회색으로
    if(s<.10) return hslToHex(h, .05, hi);
    return paletteColor(bgHex, .55, .20, hi, .78);
}
function getSubjTextColor(name){
    ...
    const bgTable = SUBJECT_BG[currentTheme] || SUBJECT_BG.clearsky;
    if(paletteKey && bgTable[paletteKey]){
        return paletteTextOn(bgTable[paletteKey]);
    }
    const key = normalizeTimelineSubject(name);
    if(bgTable[key]) return paletteTextOn(bgTable[key]);
    const baseHex = THEMES[currentTheme]?.solid || THEMES[currentTheme]?.color || THEMES.clearsky.color;
    const tint = TIMELINE_SUBJECT_TINTS[key] ?? TIMELINE_SUBJECT_TINTS['기타'];
...
function renderTimeline(){
    ...
    // ★ "공부일" 개념: 오늘 05:00 ~ 내일 04:59까지를 하루로 본다.
    //   자정을 넘겨 새벽까지 공부한 기록은 저장될 때 이미 "오늘" 날짜(timerDate) 로그에
    //   startSec 0~4:59(0~17999초) 범위로 들어가 있으므로, 그 값을 그대로 24~28:59시대로
    //   환산해서 오늘 페이지 맨 아래에 이어 그리면 된다. (다른 날짜의 로그를 끌어올 필요 없음)
    const DAY_START_SEC = 5*3600;
    if(isDayTimeline && typeof timerDate==='string'){
        allLogs = allLogs.map(l => {
            // 05시 이전(0~4:59)에 시작된 자기 날짜 기록은 자정을 넘긴 것으로 보고 +24시간 이동
            if(l.startSec < DAY_START_SEC){
                return {...l, startSec:l.startSec+86400, endSec:l.endSec+86400};
            }
            return l;
        });
    }
    ...
    const rowHeight = isDayTimeline ? 39 : 30;
    const rowMargin = 1;
    const rowRadius = 6;
    const hourLabelW = isDayTimeline ? 18 : 16;
    const hourLabelFont = isDayTimeline ? 11 : 10;
    const blockInset = 0;
    const cellFlex = isDayTimeline ? 0.76 : 0.9;

    // 컬러 맵 (범례는 표시하지 않지만 블록 색상 계산에 필요)
    const colorMap={};
    allLogs.forEach(l=>{
    ...
    for(let h=minHour;h<maxHour;h++){
        const hSec=h*HOUR;
        const hourLabel=`${String(h%24).padStart(2,'0')}`;
        // 6개 셀 (각 10분)
        ...
        rows.push(`<div style="display:flex;align-items:center;">
            <div style="text-align:right;padding-left:${isDayTimeline?5:4}px;padding-right:${isDayTimeline?5:4}px;font-size:${hourLabelFont}px;font-weight:800;color:#94a3b8;flex-shrink:0;">${hourLabel}</div>
            <div style="display:flex;flex:1;position:relative;">${cells}${isDayTimeline?renderTimelineNotesOverlay(h,hSec,HOUR):''}</div>
        </div>`);
    }
...
// ★ "오늘의 공부 흐름"에 표시하는 시간대별 메모(밥/운동 등) — 같은 시(hour) 안에서만 지원
function getTimelineNotes(dateStr){
    try{ return JSON.parse(localStorage.getItem(`young_timeline_notes_${dateStr}`))||[]; }catch{ return []; }
}
function saveTimelineNotes(dateStr, notes){
    try{ localStorage.setItem(`young_timeline_notes_${dateStr}`, JSON.stringify(notes)); }catch{}
}
function addTimelineNote(){
    const dateStr = typeof timerDate==='string' ? timerDate : TODAY;
    const input = prompt('시간과 내용을 입력하세요 (예: 12:30~13:20 운동)');
    if(!input) return;
    const parsed = parseTimelineNoteInput(input);
    if(!parsed){ showToast('⏱ 형식이 올바르지 않아요 (예: 12:30~13:20 운동)'); return; }
    const notes=getTimelineNotes(dateStr);
    notes.push(parsed);
    saveTimelineNotes(dateStr, notes);
    renderTimeline();
}
function parseTimelineNoteInput(input){
    const m = input.trim().match(/^(\d{1,2}:\d{2})\s*~\s*(\d{1,2}:\d{2})\s+(.+)$/);
    if(!m) return null;
    const toSec=(s)=>{ const [h,mi]=s.trim().split(':').map(Number); return h*3600+mi*60; };
    let startSec=toSec(m[1]), endSec=toSec(m[2]);
    if(startSec<DAY_START_TIMELINE_SEC) startSec+=86400;
    if(endSec<DAY_START_TIMELINE_SEC) endSec+=86400;
    if(endSec<=startSec) return null;
    return {label:m[3].trim(), startSec, endSec};
}
function editTimelineNote(idx){
    const dateStr = typeof timerDate==='string' ? timerDate : TODAY;
    const notes=getTimelineNotes(dateStr);
    const note=notes[idx];
    if(!note) return;
    const toHM=(sec)=>{ sec=sec%86400; return `${String(Math.floor(sec/3600)).padStart(2,'0')}:${String(Math.floor((sec%3600)/60)).padStart(2,'0')}`; };
    const input = prompt('수정할 내용을 입력하세요 (비우고 확인하면 삭제돼요)', `${toHM(note.startSec)}~${toHM(note.endSec)} ${note.label}`);
    if(input===null) return;
    if(input.trim()===''){ notes.splice(idx,1); saveTimelineNotes(dateStr, notes); renderTimeline(); return; }
    const parsed = parseTimelineNoteInput(input);
    if(!parsed){ showToast('⏱ 형식이 올바르지 않아요 (예: 12:30~13:20 운동)'); return; }
    notes[idx]=parsed;
    saveTimelineNotes(dateStr, notes);
...
function renderTimelineNotesOverlay(h, hSec, HOUR){
    const dateStr = typeof timerDate==='string' ? timerDate : TODAY;
    const notes=getTimelineNotes(dateStr);
    return notes.map((note,idx)=>{
        const startHour=Math.floor(note.startSec/3600);
        const endHour=Math.floor((note.endSec-1)/3600);
        if(h<startHour || h>endHour) return '';
        const segStart = h===startHour ? note.startSec : hSec;
        const segEnd = h===endHour ? note.endSec : hSec+HOUR;
        const leftPct=((segStart-hSec)/HOUR)*100;
        const widthPct=Math.max(((segEnd-segStart)/HOUR)*100, 6);
        const isFirst = h===startHour, isLast = h===endHour;
        const midSec = (note.startSec+note.endSec)/2;
        const showLabel = midSec>=hSec && midSec<hSec+HOUR;
        return `<div onclick="editTimelineNote(${idx})"
            style="position:absolute;left:${leftPct}%;width:${widthPct}%;top:50%;transform:translateY(-50%);z-index:5;cursor:pointer;display:flex;flex-direction:column;align-items:center;pointer-events:auto;overflow:visible;">
            <div style="width:100%;height:0;border-top:2px solid #94a3b8;position:relative;">
                ${isFirst?`<div style="position:absolute;left:-1px;top:-3px;width:0;height:0;border-top:3px solid transparent;border-bottom:3px solid transparent;border-right:5px solid #94a3b8;"></div>`:''}
                ${isLast?`<div style="position:absolute;right:-1px;top:-3px;width:0;height:0;border-top:3px solid transparent;border-bottom:3px solid transparent;border-left:5px solid #94a3b8;"></div>`:''}
            </div>
            ${showLabel?`<div style="font-size:9px;font-weight:900;color:#64748b;background:transparent;padding:0 3px;margin-top:1px;white-space:nowrap;">${note.label}</div>`:''}
        </div>`;
    }).join('');
}
const DAY_START_TIMELINE_SEC = 5*3600;
...
// ★ 오전(7:00~12:30)/오후(12:31~20:00)/밤(20:01~다음날 4:00) 시간대별 공부시간 요약
function computeDayTimeBuckets(dateStr){
    function sumRange(logs, lo, hi){
        let s=0;
        (logs||[]).forEach(l=>{
            const st=Math.max(l.startSec,lo), en=Math.min(l.endSec,hi);
            if(en>st) s+=(en-st);
        });
        return s;
    }
    let todayLogs=[];
    try{ todayLogs=JSON.parse(localStorage.getItem(`yst4_log_${dateStr}`))||[]; }catch{}
    // ★ 아직 정지 안 하고 켜져 있는(로그에 안 남은) 실시간 구간도 포함시켜야
    //   "오늘 공부 시간" 총합과 정확히 일치한다 (타임라인 차트가 라이브 구간을 그리는 것과 동일한 방식)
    if(dateStr===timerDate){
        const nowSec=getNowDaySec();
        timerSubjects.forEach(s=>{
            if(s.on && s._startSec!==undefined){
                todayLogs=[...todayLogs, {n:s.n, startSec:s._startSec, endSec:nowSec}];
            }
        });
    }
    // ★ "공부일" 개념(오늘 05:00~다음날 05:00=28:59)에 맞춰 오전(5:00~12:30)/오후(12:31~20:00)/
    //   밤(20:01~다음날 04:59=28:59)으로 하루를 빈틈없이 나눈다.
    //   자정을 넘겨 저장된 기록(startSec 0~4:59)은 이미 이 날짜(dateStr) 로그에 들어있으므로,
    //   그 값을 +24시간(86400초) 이동해 24:00~28:59 구간으로 취급해서 "밤"에 합산한다.
    const DAY_START_SEC=5*3600;
    const normalizedLogs = todayLogs.map(l => l.startSec < DAY_START_SEC
        ? {...l, startSec:l.startSec+86400, endSec:l.endSec+86400}
        : l
    );
    const morning = sumRange(normalizedLogs, 5*3600, 12*3600+30*60);
    const afternoon = sumRange(normalizedLogs, 12*3600+31*60, 20*3600);
    const night = sumRange(normalizedLogs, 20*3600+60, 29*3600); // 20:01 ~ 28:59(다음날 04:59)
    return {morning, afternoon, night, total: morning+afternoon+night};
}
function fmtHFromSec(sec){ return (sec/3600).toFixed(1)+'H'; }
function renderDayTimeBucketsLabel(dateStr){
    const b=computeDayTimeBuckets(dateStr);
    return `${fmtHFromSec(b.morning)} / ${fmtHFromSec(b.afternoon)} / ${fmtHFromSec(b.night)} = ${fmtHFromSec(b.total)}`;
}
...
function toggleTimerStart(i){
    ...
    timerSubjects=timerSubjects.map((s,j)=>{
        if(j===i) return {...s,on:willBeOn,_startSec:willBeOn?nowSec:undefined,_lastCreditedSec:undefined};
        return s.on?{...s,on:false,_startSec:undefined,_lastCreditedSec:undefined}:s;
    });
...
function renderTimeEditBlocks(){
    ...
    wrap.innerHTML=timeEditBlocks.map((b,i)=>`
        <div class="time-edit-block-row">
            <input type="text" inputmode="numeric" placeholder="HH:MM" value="${b.start}" onchange="updateTimeEditBlock(${i},'start',this.value)">
            <span class="text-slate-300 font-black">~</span>
            <input type="text" inputmode="numeric" placeholder="HH:MM" value="${b.end}" onchange="updateTimeEditBlock(${i},'end',this.value)">
...
function updateTimeEditBlock(i,field,val){
    ...
    if(normalized===null){ showToast('⏱ 시간 형식이 올바르지 않아요 (예: 12:30)'); renderTimeEditBlocks(); return; }
    timeEditBlocks[i]={...timeEditBlocks[i],[field]:normalized};
    renderTimeEditBlocks();
}
// ★ "1230", "12:3", "12.30" 등 자유롭게 입력해도 "HH:MM"으로 보정
function normalizeTimeInput(val){
    val=String(val||'').trim();
    if(!val) return '';
    let m = val.match(/^(\d{1,2})[:.\s]?(\d{2})$/);
    if(!m) m = val.match(/^(\d{1,2})$/) ? [null, val, '00'] : null;
    if(!m) return null;
    const h=+m[1], mi=+m[2];
...
function renderRecordsWeek(container){
    ...
    for(const ds of dates){
        ...
        dayRows+=`<div class="flex items-center gap-3 mb-2.5">
            <div class="text-[15px] font-black w-6 text-center ${isToday?'text-pc':'text-slate-400'}">${dayNames[d.getDay()]}</div>
            <div class="text-[15px] font-black text-slate-400 w-14">${ds.slice(5)}</div>
            ...
            <div class="text-[15px] font-black text-pc w-12 text-right">${secs>0?h:'-'}</div>
        </div>`;
    }
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="card p-5">
                <div class="flex justify-between items-center mb-5">
                    <h3 class="text-[19px] font-black text-pc">일별 공부시간</h3>
                    <div class="text-[19px] font-black text-slate-400">${toKey(ws).slice(5)} ~ ${toKey(we).slice(5)}</div>
                </div>
                ${dayRows}
                <div class="border-t border-slate-100 pt-3 mt-3 flex justify-between">
                    <span class="text-[19px] font-black text-slate-400">주간 합계</span>
                    <span class="text-[19px] font-black text-pc">${fmtHHMMSS(totalSecs)}</span>
                </div>
            </div>
            <div class="card p-5">
                <h3 class="text-[19px] font-black text-pc mb-5">과목별 공부시간</h3>
                ${renderSubjectDonutChart(sorted, totalSecs)}
            </div>
        </div>`;
}
...
/* ═══════════════════════════════════════════════
   RECORDS — MONTH VIEW
═══════════════════════════════════════════════ */
// ★ 달력 셀 배색 — 절대 시간 기준 5단계(1/4/7/10/12시간)로 고정. 기록·플래너 월간 달력이 공통으로 사용.
const CAL_HOUR_STOPS=[1,4,7,10,12];
const CAL_ALPHA_STOPS=[0.07,0.25,0.45,0.65,0.80];
function alphaForSecs(secs){
    const hrs=secs/3600;
    if(hrs<1) return 0; // 1시간 미만 — 투명
    if(hrs<4) return CAL_ALPHA_STOPS[0]; // 1h~4h미만 — 1h 색상
    if(hrs<7) return CAL_ALPHA_STOPS[1]; // 4h~7h미만 — 4h 색상
    if(hrs<10) return CAL_ALPHA_STOPS[2]; // 7h~10h미만 — 7h 색상
    if(hrs<12) return CAL_ALPHA_STOPS[3]; // 10h~12h미만 — 10h 색상
    return CAL_ALPHA_STOPS[4]; // 12h 이상 — 12h 색상
}
function calColorLegendHtml(){
    return `<div class="flex items-center gap-2 flex-wrap">
        ${CAL_ALPHA_STOPS.map((a,i)=>`<div class="flex items-center gap-1">
            <div style="width:14px;height:14px;border-radius:3px;background:${pcAlpha(a)}"></div>
            <span class="text-[10px] text-slate-400">${CAL_HOUR_STOPS[i]}h</span>
        </div>`).join('')}
    </div>`;
}
// ★ 과목별 공부시간 도넛(원) 그래프 — 기록 탭 주간/월간 공용
function renderSubjectDonutChart(sorted, totalSec){
    if(!sorted.length) return `<p class="text-base text-slate-300 text-center py-8">타이머 기록이 없습니다</p>`;
    let acc=0;
    const gradientParts=[];
    const legendRows=[];
    sorted.forEach(([n,sec])=>{
        const color=getSubjColor(n)||'var(--pc)';
        const textColor=getSubjTextColor(n)||'var(--pc)';
        const pct=totalSec>0 ? (sec/totalSec*100) : 0;
        const start=acc, end=acc+pct;
        gradientParts.push(`${color} ${start}% ${end}%`);
        acc=end;
        legendRows.push(`<div class="flex items-center gap-2 mb-2">
            <span style="width:11px;height:11px;border-radius:3px;background:${color};flex-shrink:0;"></span>
            <span class="text-[14px] font-black flex-1 truncate" style="color:${textColor}">${n}</span>
            <span class="text-[13px] font-bold text-slate-400">${Math.round(pct)}%</span>
            <span class="text-[14px] font-black w-14 text-right" style="color:${textColor}">${fmtHHMM(sec)}</span>
        </div>`);
    });
    const gradient = gradientParts.join(', ');
    return `<div class="flex items-center gap-6 flex-wrap">
        <div style="width:140px;height:140px;border-radius:50%;background:conic-gradient(${gradient});flex-shrink:0;position:relative;">
            <div style="position:absolute;inset:32px;background:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-direction:column;">
                <span class="text-[11px] font-bold text-slate-400">합계</span>
                <span class="text-[16px] font-black text-slate-700">${fmtHHMM(totalSec)}</span>
            </div>
        </div>
        <div class="flex-1 min-w-[160px]">${legendRows.join('')}</div>
    </div>`;
}
...
function renderRecordsMonth(container){
    ...
    for(let d=1;d<=days;d++){
        ...
        calHtml+=`<div class="heat-cell" style="background:${bg};${isToday?`outline:2px solid var(--pc);outline-offset:-2px;`:''}">
            <span style="font-size:10px;font-weight:${isToday?700:400};color:#0f172a">${d}</span>
            ${hText?`<span style="font-size:13px;font-weight:900;color:#0f172a">${hText}</span>`:''}
        </div>`;
    }
    calHtml+=`</div>`;

    // Legend — 절대 시간 기준(1/4/7/10/12시간)
    calHtml+=`<div class="mt-3 flex justify-end">${calColorLegendHtml()}</div>`;

    const monthTotalSec=dates.reduce((a,ds)=>a+getTotalTrackerSecs(ds),0);
    const monthAvgSec=dates.length ? Math.round(monthTotalSec/dates.length) : 0;
    const monthNames=['1월','2월','3월','4월','5월','6월','7월','8월','9월','10월','11월','12월'];
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="card p-5">
                <div class="flex justify-between items-center mb-1 flex-wrap gap-1">
                    <h3 class="font-black text-[19px] text-pc">${recordsYear}년 ${monthNames[recordsMonth]}</h3>
                    <div class="text-[19px] font-black text-slate-400">월간 합계 <span class="text-pc">${fmtHHMMSS(monthTotalSec)}</span></div>
                </div>
                <div class="text-right text-[13px] font-bold text-slate-400 mb-3">하루 평균 <span class="text-pc font-black">${fmtHHMMSS(monthAvgSec)}</span></div>
                ${calHtml}
            </div>
            <div class="card p-5">
                <h3 class="font-black text-[19px] text-pc mb-5">과목별 공부시간</h3>
                ${renderSubjectDonutChart(sorted, monthTotalSec)}
            </div>
        </div>`;
}
...
function renderRecords(){
    ...
    ['pass','lecture'].forEach(v=>{
        const el=document.getElementById(`ptab-${v}`);
        if(el){el.className='rec-tab'+(currentProgressRecordView===v?' active':'');}
    });

    // Nav buttons
    const navBtns=document.getElementById('records-nav-btns');
    const periodTabs=document.getElementById('records-period-tabs');
    const progressTabs=document.getElementById('records-progress-tabs');
    if(periodTabs) periodTabs.classList.add('hidden');
    if(progressTabs) progressTabs.classList.toggle('hidden', currentRecordMode!=='progress');
    ...
    if(currentRecordMode==='progress'){
        navBtns.innerHTML='';
        if(currentProgressRecordView==='pass') renderProgressPassTable(document.getElementById('records-container'));
        else renderProgressLectureView(document.getElementById('records-container'));
        return;
    }

    if(currentRecordMode==='study'){
        navBtns.innerHTML='';
        const container=document.getElementById('records-container');
        container.innerHTML=`
            <div id="records-period-compare" class="mb-6"></div>
            <div id="records-year-grid"></div>
            <div class="flex items-center justify-between mb-3">
                <div class="flex gap-2" id="records-month-nav"></div>
            </div>
            <div id="records-month-sub" class="mb-6"></div>
            <div class="flex items-center justify-between mb-3">
                <div class="flex gap-2" id="records-week-nav"></div>
            </div>
            <div id="records-week-sub" class="mb-6"></div>
            <div id="records-trend"></div>`;
        renderRecordsYearGrid(document.getElementById('records-year-grid'));
        renderRecordsPeriodCompare(document.getElementById('records-period-compare'));
        document.getElementById('records-month-nav').innerHTML=`
        ...
        document.getElementById('records-week-nav').innerHTML=`
            ...
            <button onclick="recordsWeekRef.setDate(recordsWeekRef.getDate()+7);renderRecords()" class="text-xs font-black text-pc px-3 py-1.5 rounded-lg hover:bg-slate-50 border border-slate-200 transition-colors">▶</button>`;
        renderRecordsMonth(document.getElementById('records-month-sub'));
        renderRecordsWeek(document.getElementById('records-week-sub'));
        renderRecordsTrend(document.getElementById('records-trend'));
        return;
    }
...
let currentProgressRecordView = 'pass'; // 'pass'(회독) | 'lecture'(인강)
function switchProgressRecordView(v){currentProgressRecordView=v;renderRecords();}
...
// ★ 실제 도달 페이지 저장
// ★ 실제 진도가 바뀌었을 때(체크 완료/실제값 입력/목표 직접 수정) 앞으로의 자동배치 계획을
//   지금 이 순간의 진도 기준으로 즉시 다시 짜고, 화면도 바로 갱신한다
function syncFutureScheduleWithActualProgress(){
    autoScheduleWeek();
    refreshEmbeddedDayView();
    renderProgress();
}
...
// ★ 어떤 날짜(dateStr)의 idx번째 태스크가 배열에서 제거됐을 때, 그 뒤에 있던 태스크들의
//   실제값(pageActuals)/완료체크(completedTasks) 기록이 같이 한 칸씩 안 밀리면 엉뚱한 태스크에
//   붙어버리거나(값이 뒤바뀜) 유실된 것처럼 보인다. 삭제 지점 이후 것들을 전부 한 칸씩 당겨준다.
function reindexDayRecordsAfterRemoval(dateStr, removedIdx){
    [completedTasks, pageActuals].forEach(store=>{
        const moved={};
        Object.keys(store).forEach(k=>{
            if(!k.startsWith(dateStr+'-')) return;
            const ti=parseInt(k.split('-').pop());
            if(ti===removedIdx){ delete store[k]; }
            else if(ti>removedIdx){ moved[`${dateStr}-${ti-1}`]=store[k]; delete store[k]; }
        });
        Object.assign(store, moved);
    });
}

// ★ 다른 날짜의 "안 한 일"을 지금 보고 있는 날짜(selectedDate)로 복사
function copyTaskToToday(dateStr, idx){
    const srcDay = scheduleData.find(d=>d.date===dateStr);
    if(!srcDay || !srcDay.tasks[idx]) return;
    const t = srcDay.tasks[idx];
    let today=scheduleData.find(d=>d.date===TODAY);
    if(!today){ today={date:TODAY, tasks:[]}; scheduleData.push(today); }
    if(today.tasks.includes(t)){ showToast('이미 오늘에 있어요'); return; }
    today.tasks.push(t);
    saveAll();
    if(selectedDate===TODAY) renderPlanner();
    showToast('✅ 오늘로 복사했어요');
}

function deleteTask(idx){
    const d=scheduleData.find(d=>d.date===selectedDate);if(!d)return;
    const removedText=d.tasks[idx];
    // ★ 삭제한 태스크가 등록된 진도 항목(의 진도 범위 태스크)과 매칭되면, 이 날짜는 다시 자동으로 채우지 않도록 기억해둔다.
    //   같은 과목+항목명이 여러 개(회독 시리즈) 등록돼 있어도 전부에 기록하고, 접두어 키로도 남겨서 새로고침 뒤에 되살아나지 않게 한다.
    if(removedText){
        const sub=getTaskSubject(removedText);
        const pages=parseTaskPages(removedText);
        const items=findAllMatchingProgressItems(sub, removedText).filter(p=>pages && pages.unit===p.unit);
        if(items.length){
            const store=getAutoScheduleStore();
            let storeChanged=false;
            items.forEach(item=>{
                [item.id, autoExclKey(item)].forEach(k=>{
                    if(!excludedAutoDates[k]) excludedAutoDates[k]=[];
                    if(!excludedAutoDates[k].includes(selectedDate)) excludedAutoDates[k].push(selectedDate);
                });
                const rec=store[item.id];
                if(rec){
                    if(!rec.gen) rec.gen={};
                    rec.gen[selectedDate]=null; // 이 날짜에는 자동 제안이 없다는 표시(같은 문구를 다시 직접 넣어도 사용자 소유로 취급)
                    storeChanged=true;
                }
            });
            if(storeChanged) saveAutoScheduleStore(store);
    ...
    reindexDayRecordsAfterRemoval(selectedDate, idx);
    saveAll();
    if(!timerInterval && timerDate===selectedDate){
        timerReady=false;
        loadTimerDay(selectedDate);
...
function saveAll(){
    ...
    localStorage.setItem('young_study_times',JSON.stringify(studyTimes));
    localStorage.setItem('young_page_actuals',JSON.stringify(pageActuals)); // ★
    localStorage.setItem('young_excluded_auto_dates', JSON.stringify(excludedAutoDates));
...
renderMonth=function(container){
    ...
    const MONTH_GOAL_HOURS=180;
    const monthGoalPct=Math.round(monthSecs/(MONTH_GOAL_HOURS*3600)*100);

   let html=`<div id="progress-summary-anchor"></div><div class="nav-box mb-5">
    <div class="nav-box-inner">
        <button onclick="viewDate.setMonth(viewDate.getMonth()-1);render()" class="nav-box-btn">◀ 이전 달</button>

        <div class="nav-box-center">
            <div class="nav-box-title flex items-center justify-center gap-2">
                <span>${getThemeAccent()} ${year}.${String(month+1).padStart(2,'0')}</span>
                ${collapseArrowHtml('toggleMonthTaskVisibility(event)', monthShowAllTasks)}
            </div>
...
<div class="flex items-center justify-end mb-2 flex-wrap gap-2">
    ${calColorLegendHtml()}
</div>`;
    ...
    for(let d=1;d<=days;d++){
        ...
        const cellBg=timeSec>0?pcAlpha(alphaForSecs(timeSec)):'#ffffff';
        ...
        html+=`<div class="cal-cell${isToday?' is-today':''}${monthShowAllTasks?' cal-cell-expanded':''}" style="background:${cellBg};" onclick="goToDay('${key}')">
            <div class="cal-cell-summary">
                ...
                ${isOff?`<span class="block ${isBareOff?'mt-3 text-[16px]':'mt-1.5 text-[10px]'} leading-none font-black tracking-wide" style="color:#94a3b8;">Day off</span>`:''}
                ${(!isOff || timeSec>0) ? `
                <span class="block mt-3 text-[16px] leading-none font-black" style="color:${timeSec>0?'color-mix(in srgb, var(--pc) 80%, black 20%)':'#cbd5e1'}">${timeSec>0?fmtHHMM(timeSec):'00:00'}</span>
                <span class="text-[19px] font-black" style="color:${progressPct>0?'color-mix(in srgb, var(--pc) 80%, black 20%)':'#cbd5e1'}">${progressPct}%</span>` : ''}
            </div>${taskList}
        </div>`;
    }
    html+=`</div></div>`;
    container.innerHTML=html;
    positionProgressSummary('progress-summary-anchor');
};
...
const conditionCardHtml = `<div class="card p-4 mb-4">
        <h3 class="text-[14px] font-black text-slate-500 mb-2 flex items-center gap-1.5">
            ${collapseArrowHtml('toggleConditionCardCollapse()', !conditionCardCollapsed)}
            ${dayLabelForCard} 컨디션 — 이 날 목표 추천이 이만큼 바뀌어요
        </h3>
        <div style="${conditionCardCollapsed?'display:none':''}">
  ...
  let html=`${conditionCardHtml}<div id="progress-summary-anchor"></div><div class="nav-box mb-5">
    <div class="nav-box-inner">
...
<div class="day-main-grid">
    <div class="space-y-4">
        <div class="card day-task-card">
            <div class="flex gap-2 mb-4 items-center">
                <div class="flex flex-col justify-between shrink-0" style="align-self:stretch;padding:3px 0;">
                    ${collapseArrowHtml('toggleDayTasksCollapse()', !dayTasksCollapsed, 'width:16px;height:11px;')}
                    <button onclick="toggleTaskControlsCollapse()" title="목표/실제 입력칸 접기·펼치기" class="shrink-0" style="background:transparent;border:none;color:#cbd5e1;font-size:10px;font-weight:900;cursor:pointer;width:16px;height:11px;line-height:1;">${taskControlsCollapsed?'▤':'▦'}</button>
                </div>
                <input id="new-task-input" type="text" placeholder="[과목] 할 일을 입력"
                    class="flex-1 min-w-0 bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 outline-none focus:border-pc font-semibold transition-colors"
                    style="font-size:17px;height:44px;"
                    onkeydown="if(event.key==='Enter')addTask()">
                <button onclick="addTask()" class="shrink-0 bg-pc text-white px-5 rounded-2xl font-black hover:opacity-80 transition-all" style="font-size:15px;height:44px;">추가</button>
            </div>
            <div>`;
    ...
    day.tasks.forEach((t,idx)=>{
        ...
        // ★ "쉬는 과목이에요" / "추천 오늘 목표" 힌트 문구는 더 이상 표시하지 않음 (요청에 따라 제거)
        const suggestionHtml='';
        const checkboxHtml = !pages ? `<input type="checkbox" ${isDone?'checked':''} onchange="toggleTask('${day.date}',${idx})" class="cursor-pointer shrink-0 day-task-checkbox">` : '';
        const goalHtml = pages ? `<span class="day-task-meta day-task-goal-wrap" style="${taskControlsCollapsed?'display:none':''}">
        ...
        const slashHtml = pages ? `<span class="day-task-meta" style="margin:0;">/</span>` : '';
        const actualHtml = pages ? `<span class="day-task-page-meta day-task-boxes" style="gap:0;">
                <input type="number" value="${actualVal}" placeholder="실제"
                    onchange="setPageActual('${actualKey}',this.value)"
                    class="day-task-actual"><span class="day-task-meta" style="margin-left:0;">${pages.unit}</span>
            </span>` : '';
        const percentHtml=`<span class="day-task-percent" style="margin-left:4px;">${pages?pct:(isDone?100:0)}%</span>`;
        const copyToTodayHtml = selectedDate!==TODAY ? `<button onclick="copyTaskToToday('${day.date}',${idx})" title="오늘로 복사" style="color:#cbd5e1;font-size:12px;font-weight:900;background:transparent;border:none;cursor:pointer;padding:0 2px;">→</button>` : '';
        const isColored = pct>=100;
        const overCapBg = pct===100 ? `background:${pcAlpha(0.10)};border-radius:__RAD__;` // ★ 100% 정확히 달성 — 테마색 기준
            : pct>100 ? `background:${pcAlpha(0.22)};border-radius:__RAD__;` // ★ 100% 초과 — 테마색 기준
            : '';
        const rowHtml=`<div class="day-task-row day-task-row-grouped ${isDone?'is-done':''}" style="${overCapBg}">
            <div class="shrink-0" style="width:2px;"></div>
            <div class="min-w-0">
                <span class="day-task-title ${isDone?'line-through text-slate-400':'text-slate-700'}">${txt||t}</span>
                ${suggestionHtml}
            </div>
            <div class="day-task-controls" style="padding-right:0;">
                ${goalHtml}
                ${slashHtml}
                ${actualHtml}
                ${copyToTodayHtml}
            ...
            <button onclick="deleteTask(${idx})" title="삭제" class="hover:text-red-400 rounded transition-colors shrink-0" style="color:#cbd5e1;font-size:12px;font-weight:900;padding:2px 2px 2px 4px;">✕</button>
        </div>`;
    ...
    dayGroupKeys.forEach(groupKey=>{
        ...
        const timerBtn=timerItem
            ...
            : '';
        const editBtn=`<button onclick="openTimeEditModal(${timerIdx})" class="day-subject-edit-btn" title="시간 수정"${timerIdx<0?' disabled':''}>⏱</button>`;
        // ★ 이 과목의 "모든" 하위 항목이 100% 이상일 때만 그룹에도 배경을 준다.
        //   그중 하나라도 100%를 "초과"했으면 그룹은 2색(오늘 요일과 동일), 전부 정확히 100%면 1색(미래 요일과 동일).
        const allItemsAtLeast100 = g.pcts.length>0 && g.pcts.every(p=>p>=100);
        const anyItemOver100 = g.pcts.some(p=>p>100);
        const groupOverCapBg = !allItemsAtLeast100 ? ''
            : anyItemOver100 ? `background:${pcAlpha(0.22)};border-radius:14px;`
            : `background:${pcAlpha(0.10)};border-radius:14px;`;
        const headerSeamStyle = groupOverCapBg ? 'border-bottom:none;margin-bottom:0;' : '';
        html+=`<div class="day-subject-group">
            <div class="day-subject-header" style="${groupOverCapBg}${headerSeamStyle}${groupOverCapBg?'border-radius:14px 14px 0 0;':''}">
            ...
            <div class="day-subject-tasks" style="${dayTasksCollapsed?'display:none':''}">
                ${g.rows.map((rowHtml,i)=>{
                    if(!rowHtml.includes('__RAD__')) return rowHtml;
                    const prevColored = (i>0 && g.pcts[i-1]>=100) || (i===0 && !!groupOverCapBg);
                    const nextColored = i<g.rows.length-1 && g.pcts[i+1]>=100;
                    const topR = prevColored ? '0' : '14px';
                    const botR = nextColored ? '0' : '14px';
                    return rowHtml.replace('__RAD__', `${topR} ${topR} ${botR} ${botR}`);
                }).join('')}
            </div>
        </div>`;
    });
    ...
    <div class="space-y-4">
        <div class="card timeline-focus-card">
            <div class="flex items-center justify-between" style="min-height:0;padding-top:8px;padding-left:10px;">
                <h3 class="panel-title" style="color:var(--pc);font-size:17.5px;margin:0;">오늘의 공부 흐름</h3>
                <button onclick="addTimelineNote()" title="밥/운동 등 메모 추가" class="shrink-0" style="background:transparent;border:none;color:#94a3b8;font-size:20px;font-weight:900;cursor:pointer;line-height:1;padding:2px 8px;">+</button>
            </div>
            <div class="text-[11.5px] font-bold text-slate-400 text-right" style="margin-top:0;">${renderDayTimeBucketsLabel(selectedDate)}</div>
            <div id="timeline-chart" data-mode="day" class="mt-1"></div>
        </div>
    </div>
</div>
</div>`;
    container.innerHTML=html;
    positionProgressSummary('progress-summary-anchor');
    setTimeout(()=>{ renderTimeline(); autoFitTaskTitles(); },0);
};

// ★ 항목 제목이 한 줄 폭을 넘치면(줄바꿈/잘림 대신) 폰트 크기를 조금씩 줄여서 한 줄에 딱 맞춘다
function autoFitTaskTitles(){
    document.querySelectorAll('.day-task-title').forEach(el=>{
        let fontSize=17;
...
renderPlanner=function(){
    ...
    positionProgressSummary('progress-summary-default-anchor'); // ★ 컨테이너 비우기 전에 안전한 곳으로 먼저 이동
    c.innerHTML='';
