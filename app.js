...
function computeDayTimeBuckets(dateStr){
    ...
    // ★ 오전(4:00~12:30)/오후(12:31~20:00)/밤(20:01~다음날 4:00)으로 하루 24시간을 빈틈없이 나눠서
    //   세 구간의 합이 항상 그날의 전체 공부시간과 정확히 일치하도록 한다.
    const morning = sumRange(todayLogs, 4*3600, 12*3600+30*60);
    const afternoon = sumRange(todayLogs, 12*3600+31*60, 20*3600);
    const night = sumRange(todayLogs, 20*3600+60, 86400) + sumRange(nextLogs, 0, 4*3600);
...
<div class="day-main-grid">
    ...
    day.tasks.forEach((t,idx)=>{
        ...
        const capColorBase = subjectColor || 'var(--pc-solid)';
        const overCapBg = pct===100 ? `background:${hexToRgba(capColorBase,0.10)};border-radius:__RAD__;` // ★ 100% 정확히 달성 — 과목 고유색 기준
            : pct>100 ? `background:${hexToRgba(capColorBase,0.22)};border-radius:__RAD__;` // ★ 100% 초과 — 과목 고유색 기준
            : '';
    ...
    dayGroupKeys.forEach(groupKey=>{
        ...
        const groupOverCapBg = !allItemsAtLeast100 ? ''
            : anyItemOver100 ? `background:${hexToRgba(subjectColor,0.22)};border-radius:14px;`
            : `background:${hexToRgba(subjectColor,0.10)};border-radius:14px;`;
