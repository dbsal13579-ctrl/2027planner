...
const CAL_ALPHA_STOPS=[0.15,0.35,0.55,0.75,0.90];
    ...
    for(let d=1;d<=days;d++){
        ...
        html+=`<div class="cal-cell${isToday?' is-today':''}${monthShowAllTasks?' cal-cell-expanded':''}" style="background:${cellBg};" onclick="goToDay('${key}')">
            <div class="cal-cell-summary">
                ...
                ${(!isOff || timeSec>0) ? `
                <span class="block mt-3 text-[16px] leading-none font-black" style="color:${timeSec>0?'color-mix(in srgb, var(--pc) 85%, black 15%)':'#cbd5e1'}">${timeSec>0?fmtHHMM(timeSec):'00:00'}</span>
                <span class="text-[19px] font-black" style="color:${progressPct>0?'color-mix(in srgb, var(--pc) 85%, black 15%)':'#cbd5e1'}">${progressPct}%</span>` : ''}
            </div>${taskList}
        </div>`;
    }
    html+=`</div></div>`;
...
<div class="day-main-grid">
    ...
    day.tasks.forEach((t,idx)=>{
        ...
        const overCapBg = pct===100 ? `background:${pcAlpha(0.10)};` // ★ 100% 정확히 달성
            : pct>100 ? `background:${pcAlpha(0.30)};` // ★ 100% 초과
            : '';
    ...
    dayGroupKeys.forEach(groupKey=>{
        ...
        const groupOverCapBg = !allItemsAtLeast100 ? ''
            : anyItemOver100 ? `background:${pcAlpha(0.30)};`
            : `background:${pcAlpha(0.10)};`;
    ...
    <div class="space-y-4">
        <div class="card timeline-focus-card">
            <div class="flex items-center" style="min-height:40px;">
                <h3 class="panel-title" style="color:var(--pc);font-size:17px;">오늘의 공부 흐름</h3>
            </div>
            <div class="text-[13px] font-bold text-slate-400 text-right" style="margin-top:0;">${renderDayTimeBucketsLabel(selectedDate)}</div>
            <div id="timeline-chart" data-mode="day" class="mt-2"></div>
        </div>
    </div>
</div>
</div>`;
