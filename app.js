...
function renderTimeline(){
    ...
    const rowMargin = 2;
    const rowRadius = 6;
    const hourLabelW = isDayTimeline ? 34 : 30;
...
const CAL_ALPHA_STOPS=[0.10,0.35,0.55,0.72,0.88];
    ...
    for(let d=1;d<=days;d++){
        ...
        html+=`<div class="cal-cell${isToday?' is-today':''}${monthShowAllTasks?' cal-cell-expanded':''}" style="background:${cellBg};" onclick="goToDay('${key}')">
            <div class="cal-cell-summary">
                ...
                ${(!isOff || timeSec>0) ? `
                <span class="block mt-3 text-[16px] leading-none font-black ${timeSec>0?'text-pc':'text-slate-300'}">${timeSec>0?fmtHHMM(timeSec):'00:00'}</span>
                <span class="text-[19px] font-black ${progressPct>0?'text-pc':'text-slate-300'}">${progressPct}%</span>` : ''}
            </div>${taskList}
        </div>`;
    }
    html+=`</div></div>`;
...
<div class="day-main-grid">
    <div class="space-y-4">
        <div class="card day-task-card">
            <div class="flex gap-2 mb-4 items-center">
                ...
                <button onclick="addTask()" class="shrink-0 bg-pc text-white px-5 rounded-2xl font-black hover:opacity-80 transition-all" style="font-size:15px;">추가</button>
            </div>
            <div>`;
    ...
    day.tasks.forEach((t,idx)=>{
        ...
        const actualHtml = pages ? `<span class="day-task-page-meta day-task-boxes" style="gap:0;">
                <input type="number" value="${actualVal}" placeholder="실제"
                    ...
                    class="day-task-actual"><span class="day-task-meta" style="margin-left:2px;">${pages.unit}</span>
            </span>` : '';
        const percentHtml=`<span class="day-task-percent" style="margin-left:4px;">${pages?pct:(isDone?100:0)}%</span>`;
        const overCapBg = pct===100 ? `background:${pcAlpha(0.05)};` // ★ 100% 정확히 달성 = 미래(아직 안 온) 요일과 같은 배경
            : pct>100 ? 'background:linear-gradient(180deg, rgba(var(--pc-rgb),.11) 0%, rgba(var(--pc-rgb),.05) 100%);' // ★ 100% 초과 = 오늘 요일과 같은 배경
            : '';
    ...
    dayGroupKeys.forEach(groupKey=>{
        ...
        const groupOverCapBg = !allItemsAtLeast100 ? ''
            : anyItemOver100 ? 'background:linear-gradient(180deg, rgba(var(--pc-rgb),.11) 0%, rgba(var(--pc-rgb),.05) 100%);'
            : `background:${pcAlpha(0.05)};`;
        html+=`<div class="day-subject-group" style="${groupOverCapBg}">
    ...
    <div class="space-y-4">
        <div class="card timeline-focus-card">
            <div class="flex items-center" style="min-height:40px;">
                <h3 class="panel-title" style="color:var(--pc);font-size:22px;">오늘의 공부 흐름</h3>
            </div>
            <div class="text-[13px] font-bold text-slate-400 text-right mt-1">${renderDayTimeBucketsLabel(selectedDate)}</div>

            <div id="timeline-chart" data-mode="day" class="mt-4"></div>
        </div>
    </div>
</div>
</div>`;
