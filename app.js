...
function getNowDaySec(){
    // ★ 자정을 넘겨도(예: 밤 11시에 켠 타이머가 새벽 2시까지 계속 켜져 있는 경우)
    //   "지금" 좌표가 시작 시각보다 작아지지 않도록, 자정 이후(00:00~04:59)에는
    //   +24시간을 더해 계속 증가하는 값으로 돌려준다.
    //   (그냥 시:분:초만 반환하면 자정에 0으로 리셋되어 endSec < startSec이 되고,
    //    그 결과 05시 이전 필터에 걸려 실시간 새벽 구간이 통째로 안 그려지거나
    //    타임라인의 시간 순서가 깨지는 문제가 있었음)
    const now=new Date();
    let sec=now.getHours()*3600+now.getMinutes()*60+now.getSeconds();
    if(sec<5*3600) sec+=86400; // 00:00~04:59는 24:00~28:59로 취급
    return sec;
}
...
function renderTimeline(){
    ...
    //   ※ nowSec(getNowDaySec())이 이미 자정 이후엔 +24시간 보정된 값이므로 별도 보정 불필요
