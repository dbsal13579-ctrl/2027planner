...
(function(){
    ...
    // 이 키들은 로그인/토큰 정보라서 클라우드로 보내지 않고 이 기기에만 저장
    const EXCLUDED_KEYS = ['sync_auth_uid','sync_auth_idtoken','sync_auth_refreshtoken','sync_auth_email'];
    ...
    function schedulePush(){
        ...
        pushTimer = setTimeout(async ()=>{
            try{
                await pushCloudData(snapshotLocalStorage());
    ...
    async function hydrateAndLaunch(){
        ...
        try{
            ...
            if(cloud) applySnapshotToLocalStorage(cloud);
            else await pushCloudData(snapshotLocalStorage());
            syncEnabled = true;
        }catch(e){
